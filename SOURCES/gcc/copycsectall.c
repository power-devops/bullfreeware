...
main (int argc, char *argv[])
{
  ...
  long size, nsyms, obj, i;
  long offset, scnlen, totlen;
  ...
  csect = NULL;
  totlen = 0;
  for (obj = 3; obj < argc - 1; obj++)
    {
      ibfd = bfd_openr (argv[obj], NULL);
      if (ibfd == NULL)
        {
      ...
      if (scnlen)
        {
          totlen += scnlen;
          csect = realloc (csect, totlen);
          if (csect == NULL)
            {
              fprintf (stderr, "malloc csect\n");
              return 1;
            }
          if (!bfd_get_section_contents (ibfd, isect, (char *)csect + totlen - scnlen, offset, scnlen))
            {
  ...
  obfd = bfd_openw (argv[argc - 1], bfd_get_target (ibfd));
  if (obfd == NULL)
    {
  ...
  if (!bfd_set_section_size (obfd, osect, totlen))
    {
      fprintf (stderr, "bfd_set_section_size\n");
      return 1;
    }
  if (!bfd_set_section_contents (obfd, osect, csect, 0, totlen))
    {
  ...
  isym->value = 0;
  /* Export the symbol. */
  coffsymbol (isym)->native->u.syment.n_sclass = C_EXT;
  /* Change storage mapping class from XMC_XO to XMC_RO. */
  coffsymbol (isym)->native[1].u.auxent.x_csect.x_smclas = XMC_RO;
  /* Write the new CSECT length. */
  coffsymbol (isym)->native[1].u.auxent.x_csect.x_scnlen.l = totlen;
  osymtab[0] = isym;
  osymtab[1] = NULL;
