...
Patch1: %{name}-7.4.2-aix-build.patch
# Is the following necessary - SIZE_MAX 32/64 bits is defined in stdint.h
Patch2: %{name}-7.2.19-aix-network.patch
Patch3: %{name}-7.3.10-aix-setrlimit.patch
Patch4: %{name}-7.3.10-aix-tests.patch
Patch5: %{name}-7.4.2-aix-trailing-slash.patch
Patch6: %{name}-7.4.2-phpini.patch
Patch7: %{name}-7.2.24-fpm.patch
...
# All files licensed under PHP version 3.01
# Not sure this is still true: bundled libgd is licensed under BSD
License: PHP
Requires: php-common%{?_isa} = %{version}-%{release}
BuildRequires: gd-devel >= 2.1.1
...
if [ -e ext/soap/.libs/soap.so ]; then
 cp ext/soap/.libs/soap.so modules/
fi
if [ -e ext/gd/.libs/gd.so ]; then
 cp ext/gd/.libs/gd.so modules/
fi
...
# For 32 bit build-cgi and build-apache originally, 64 bit TBC
...
# Not sure why diasbled for 64 bits (--disable-opcache TBC)  --enable-opcache=no
...
#for TARGET in install-cli install-build install-headers install-programs install-fpm; do
...
/usr/bin/strip -X64 ${RPM_BUILD_ROOT}%{_sbindir}/php-fpm
mv ${RPM_BUILD_ROOT}%{_sbindir}/php-fpm ${RPM_BUILD_ROOT}%{_sbindir}/php-fpm_64
...
/usr/bin/strip  -X32 ${RPM_BUILD_ROOT}%{_sbindir}/php-fpm
mv ${RPM_BUILD_ROOT}%{_sbindir}/php-fpm ${RPM_BUILD_ROOT}%{_sbindir}/php-fpm_32
(
	cd ${RPM_BUILD_ROOT}%{_sbindir}
	ln -s php-fpm_64 php-fpm
)
...
echo "64bit php uses /etc/php64.ini"
echo ""
...
echo "extension=soap.so" >> /etc/php.ini
echo "extension=soap.so" >> /etc/php64.ini
...
%files gd
%defattr(-,root,system)
%config(noreplace) %{_sysconfdir}/php.d/gd.ini
%{_libdir}/php/modules/gd.so
%{_libdir64}/php/modules/gd.so
