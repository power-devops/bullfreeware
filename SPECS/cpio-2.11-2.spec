...
then
    if test "X`type %{DEFCC} 2>/dev/null`" != 'X'; then
       export CC="%{DEFCC} -qcpluscmt -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE"
    else 
       export CC=gcc
    fi
fi
...
export CC="/usr/vac/bin/xlc_r -qcpluscmt -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE"
...
%changelog
* Thu Mar 21 2014 Gerard Visiedo <gerard.visiedo@bull.net> 2.11-2
- Rebuild with gettext-0.17-8
