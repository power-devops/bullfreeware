...
# Do not submit
Patch1: gcc-11.1.0-configure-match-AIX-major-version.patch

# Add GO to official languages.
...
# Add gcc libpath to go binaries
Patch3:  gcc-9.3.0-gotools-fix-Makefile-v3.patch
# Default Go commands to 64bit
Patch4:  gcc-9.1.0-gotools-64bits-commands.patch

# Speedup libbacktrace for Go tracebacks
Patch5:  gcc-9.1.0-libbacktrace-speed-up-XCOFF.patch

# Default to DWARF2
Patch6:  gcc-11.1.0-aix-Switch-AIX-configurtion-to-DWARF2-debugging.patch

# Use /proc/pid/object/a.out before /proc/self/exe
# Fix panic failures.
# Don't submit
# Check without.
Patch7:  gcc-11.1.0-libbacktrace-open-with-pid-first.patch

# Add libstdc++ locale support
Patch81: gcc-11.1.0-libstdc-implement-locale-support-for-XPG7.patch
Patch82: gcc-11.1.0-libstdc-configure-after-locale.patch

# Fix 64bit inodes with stat
Patch9: gcc-11.2.0-aix-handle-64bit-inodes-for-include-directories.patch

# Fix DWARF in libbacktrace
Patch10: gcc-11.1.0-libbacktrace-fix-DWARF-suppport-for-XCOFF-files.patch

# Remove -fdump in collect2
# Do not submit for now.
Patch11: gcc-10.3.0-collect2-avoid-passing-fdump-options-when-building-t.patch

# Patch for gcc 64
Patch40: gcc-10.3.0-libstdc-Remove-GLIBCXX_CHECK_INT64_T-checks.patch
Patch41: gcc-11.2.0-_GLIBCXX_USE_INT128-autoconf-macro.patch
Patch42: gcc-11.2.0-Make-profitability-calculation-of-RA-conflict-presen.patch
Patch43: gcc-11.2.0-aix-adjust-installation-directories-for-GCC64.patch
Patch44: gcc-11.2.0-configure-adjust-installation-directory-for-AIX.patch
Patch45: gcc-11.2.0-aix-Add-FAT-library-support-for-libffi.patch

# Patch for 7.3
Patch50: gcc-11.2.0-aix-Add-AIX-7.3-configuration-and-SPDX-License-Ident.patch
...
# Fix libgo tests.
Patch103:   gcc-9.1.0-libgo-testsuite-add-mcmodel-large-for-net-http-tests.patch

# Force no inline for runtime.callers as libbacktrace isn't perfect on AIX
# Check without
Patch104:   gcc-10.1.0-libgo-go-runtime-add-noinline-for-callers.patch

# Fix syscall test compilation
Patch105:   gcc-10.1.0-libgo-go-syscall-fix-syscall_unix_test.go-compilation.patch

# Fail with a huge trace. Infinite recursions seesm to trigger a out-of-memory.
Patch106:   gcc-11.2.0-libgo-force-fail-on-TestFcntlFlock.patch
...
# Workaround for DWARF relocations made by AIX linker
BuildRequires: util-aix-ld
...
%define issue27836dir gcc/testsuite/go.test/test/fixedbugs/issue27836.dir
mv %{issue27836dir}/*foo.go %{issue27836dir}/Äfoo.go || :
mv %{issue27836dir}/*main.go %{issue27836dir}/Ämain.go || :
...
# Even if IRA bug (cf PR 102147) have been fixed,
# there is still another hidden bug, triggering differences
# between stages.
%if %{default_arch} == 64
export CC="$CC -maix64"
export CXX="$CXX -maix64"
%endif
...
# Fix DWARF relocations and rebuild archives.
%ifnos aix7.3
%if %{default_arch} == 64
fix-ld-dwarf.sh %{buildhost}/pthread/ppc32/libstdc++-v3/src/.libs/libstdc++.so.*
/usr/bin/ar -X32 rc %{buildhost}/pthread/ppc32/libstdc++-v3/src/.libs/libstdc++.a \
	    %{buildhost}/pthread/ppc32/libstdc++-v3/src/.libs/libstdc++.so.*

fix-ld-dwarf.sh %{buildhost}/ppc32/libgo/.libs/libgo.so.*
fix-ld-dwarf.sh %{buildhost}/pthread/ppc32/libgo/.libs/libgo.so.*
/usr/bin/ar -X32 rc %{buildhost}/ppc32/libgo/.libs/libgo.a \
	    %{buildhost}/ppc32/libgo/.libs/libgo.so.*
/usr/bin/ar -X32 rc %{buildhost}/pthread/ppc32/libgo/.libs/libgo.a \
	    %{buildhost}/pthread/ppc32/libgo/.libs/libgo.so.*
%else
fix-ld-dwarf.sh %{buildhost}/pthread/libstdc++-v3/src/.libs/libstdc++.so.*
/usr/bin/ar -X32 rc %{buildhost}/pthread/libstdc++-v3/src/.libs/libstdc++.a \
	    %{buildhost}/pthread/libstdc++-v3/src/.libs/libstdc++.so.*

fix-ld-dwarf.sh %{buildhost}/libgo/.libs/libgo.so.*
fix-ld-dwarf.sh %{buildhost}/pthread/libgo/.libs/libgo.so.*
/usr/bin/ar -X32 rc %{buildhost}/libgo/.libs/libgo.a \
	    %{buildhost}/libgo/.libs/libgo.so.*
/usr/bin/ar -X32 rc %{buildhost}/pthread/libgo/.libs/libgo.a \
	    %{buildhost}/pthread/libgo/.libs/libgo.so.*
%endif
%endif
...
# .gox files
%{gcclibdir}/go
%{gcclibdir}/pthread/go
%{gcclibdir}/ppc64/go
%{gcclibdir}/pthread/ppc64/go
%if %{default_arch} == 64
%{gcclibdir}/ppc32/go
%{gcclibdir}/pthread/ppc32/go
%endif


%changelog
* Tue Oct 12 2021 Clement Chigot <clement.chigot@atos.net> - gcc11-11.2.0-1
- Update to version 11.2.0 and create new package gcc11
- Fully enable 64bit support
