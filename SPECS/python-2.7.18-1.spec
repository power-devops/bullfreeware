...
%description
Python 2.7.18 will be the last Python 2 version.
Consider to update to Python 3.
...
%description db
The DB module provides library to work with Berkeley DB.
...
echo "Python 2.7.18 will the last Python 2 version."
echo "Consider update to Python 3."
...
%exclude %{_libdir}/%{name}%{BASE_VERSION}/lib-dynload/_bsddb.so
...
%exclude %{_libdir64}/%{name}%{BASE_VERSION}/lib-dynload/_bsddb.so
...
%files db
%defattr(-,root,system,-)
%{_libdir}/%{name}%{BASE_VERSION}/lib-dynload/_bsddb.so
%{_libdir64}/%{name}%{BASE_VERSION}/lib-dynload/_bsddb.so
...
#################################
%changelog
* Fri Oct 23 2020 Étienne Guesnet <etienne.guesnetatos.net> - 2.7.18-1
- New version 2.7.18
- Put DB module in a subpackage

* Mon Oct 19 2020 Michael Wilson <michael.a.wilson@atos.net> - 2.7.17-3
- Rebuild with CC / CXX for GCC /opt/freeware/bin/gcc  - symlinks removed in GCC
