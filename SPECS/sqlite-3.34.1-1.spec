...
Source1: http://www.sqlite.org/%{year}/sqlite-doc-%{docrealver}.zip
Source1000:	%{name}-%{version}-%{release}.build.log

# Support a system-wide lemon template
#Patch1:  %{name}-%{version}-lemon-system-template.patch
Patch1:  %{name}-3.6.23-lemon-system-template.patch
...
Patch2:  %{name}-3.30.1-Makefile-add-LDFLAGS-for-lemon.patch
...
# /usr/bin/unzip doesn't exist anymore
%define __unzip /opt/freeware/bin/unzip
...
# Some tests (64 bits) require large files or memory
# e.g.  bitvec-1.30.big_and_slow view-21.1 view-21.2
# But user rpmuser for autobuild cannot run the ulimit commands
# and user root is excluded from running the tests which may need these limits
# File size
# ulimit -f unlimited
# Memory size
# ulimit -m unlimited
...
%changelog
* Fri Oct 01 2021 Clément Chigot <clement.chigot@atos.net> - 3.34.1-1

* Mon Aug 31 2020 Michael Wwilson <michael.a.wilson@atos.net> - 3.33.0-1
- Updated to version 3.33.0
- Include %{_bindir}/sqldiff in main sqlite rpm
- To run all tests in veryquick.test user must not be root
