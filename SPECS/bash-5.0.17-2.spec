...
Source0: ftp://ftp.gnu.org/gnu/bash/%{name}-%{baseversion}.tar.gz
# Source1: ftp://ftp.gnu.org/gnu/bash/%{name}-%{baseversion}.tar.gz.sig
...
Patch901: bash-4.4-locale_make_check-v2.patch

# Fix the issue with test -x when root
Patch902: bash-4.test-x-root.patch

# Fix linking with shared libraries
Patch903:   %{name}-configure-fix-shrext-for-AIX-without-brtl.patch
Patch904:   %{name}-configure-fix-disable-rpath.patch

# Fix procsubs.tests
Patch905:   %{name}-tests-fix-procsubs-tests-on-OSes-using-named-pipes.patch

# When a shell opens a named pipes for its children, it might happen that one child
# try to open it while the other ones doesn't do anything with it.
# This will result into a child being unkilled. Therefore, force the
# parent process to open everyone of its created pipes before closing.
Patch906:   %{name}-5.0.17-process-substitution-unlink-named-pipes-when-closing.patch

# Some tests use a wrong locale name, or have wrong output for AIX.
Patch907:   %{name}-5.0-test.patch


#BuildRequires: patch, gettext-devel
...
# Official upstream patches
%{lua:for i=1,17 do print(string.format("%%patch%u -p1\n", i, i)) end}

# Other patches
# Locale names for UTF8 for make check on AIX are all capitals, eg EN_US.UTF-8

%patch901 -p1 -b .locale_make_check
%patch902 -p1 -b .test-x-root
%patch903 -p1
%patch904 -p1
%patch905 -p1
%patch906 -p1
