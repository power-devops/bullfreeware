...
# %%svn_python_sitearch is not defined...
...
%if %{with perl}
%package -n perl%{perl_version}-subversion
Summary: Perl bindings to the Subversion libraries
Requires: %perl_compat
Requires: subversion = %{version}-%{release}

%description -n perl%{perl_version}-subversion
...
Requires: subversion = %{version}-%{release}
Requires: perl%{perl_version}-subversion = %{version}-%{release}
Obsoletes: subversion-perl <= 1.13.0-1
...
build_svn %{_libdir64} %{swigdirs3_64} %{ruby_vendorarchdir_64}
...
for f in emacs/site-lisp xemacs/site-packages/lisp; do
    ...
    install -m 644 %{SOURCE4} ${RPM_BUILD_ROOT}%{_datadir}/$f
done

install -m 644 %{SOURCE5} ${RPM_BUILD_ROOT}%{_datadir}/emacs/site-lisp
...
/opt/freeware/bin/sed -i -e 's|#!/usr/bin/python|#!/usr/bin/env python3|' tools/examples/SvnCLBrowse
...
check_svn () {
    ...
    echo "####### Base test #######"
    gmake check CLEANUP=yes || true
    %if %{with perl}
    echo "####### Perl test #######"
    gmake check-swig-pl || true
    %endif
    %if %{with ruby}
    echo "####### Ruby test #######"
    gmake check-swig-rb || true
    %endif
    %if %{with pyswig}
    echo "####### Python test #######"
    gmake check-swig-py || true
    %endif
    # check-swig-rb omitted: it runs svnserve
    %if %{with_java}
    echo "####### Java test #######"
    gmake check-javahl || true
    %endif
}
...
%{_libdir64}/httpd/modules/mod_*.so
%if "%{_httpd_modconfdir}" != "%{_httpd_confdir}"
%doc example.conf
%endif
%endif
...
%if %{with perl}
%files -n perl%{perl_version}-subversion
%defattr(-,root,system,-)
...
%files ruby
%defattr(-,root,system,-)
%{_libdir64}/libsvn_swig_ruby*
%{ruby_vendorarchdir}/svn
%endif


%changelog
* Tue Nov 09 2021 Bullfreeware Continous Integration <bullfreeware@atos.net> - 1.14.1-1
- Update to 1.14.1

* Mon Nov 08 2021 Clément Chigot <clement.chigot@atos.net> - 1.13.0-3
- Fix Source0 URL.
- Remove unused sources.

* Tue Nov 02 2021 Étienne Guesnet <etienne.guesnet@atos.net> - 1.13.0-2
- Comment perl.
- Remove python2
- Remove 32 bits
- OK for automation
