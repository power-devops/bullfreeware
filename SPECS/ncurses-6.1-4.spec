...
(
	...
	do
		...
		(
			cd ${RPM_BUILD_ROOT}%{_libdir}
			${AR} -rv ${library}.a ${library}.so.%{MY_ABI}
			${AR} -q ${library}.a ${RPM_BUILD_ROOT}%{_libdir64}/${library}.so.%{MY_ABI}
		)
		...
		(
			case ${library} in
				libncurses|libpanel)
					cd ${RPM_BUILD_ROOT}%{_libdir}
					ln -sf ${library}.so.%{MY_ABI} ${library}.so
					cd ${RPM_BUILD_ROOT}%{_libdir64}
					ln -sf ${library}.so.%{MY_ABI} ${library}.so
					;;
				*)
					rm ${RPM_BUILD_ROOT}%{_libdir}/${library}.so.%{MY_ABI}
					rm ${RPM_BUILD_ROOT}%{_libdir64}/${library}.so.%{MY_ABI}
					;;
			esac
...
# Tests are to be run manually in tests directory
