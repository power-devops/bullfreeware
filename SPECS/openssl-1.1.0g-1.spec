...
Release: %{baserelease}%{!?nosslv2:withsslv2}%{?sub_release}
...
# Taken from 1.0.1u (with -bexpall):
...
# Taken from 1.0.2m-1withsslv2 (with -bexpall):
...
# $SOURCES/openssl-1.1.0g-1withsslv2.build.log
# $SOURCES/openssl-1.1.0g-1.build.log
...
%else
    # xlc
    ${CC} -qmkshrobj *.o -o ../libcrypto.so.%{opensslversion} -bernotok
%endif
cd ..
...
# Provide lib*.so symlinking to 1.0.2 version !!
...
rm -f libcrypto.a
rm -f libssl.a
ln -s %{_libdir}/libcrypto.a libcrypto.a
ln -s %{_libdir}/libssl.a    libssl.a
...
# (dependency on libssl.so leads to confusion between versions 1.0 and 1.1 !!)
...
%changelog
* Tue Nov 21 2017 Tony Reix <tony.reix@atos.net> - 1.1.0g-1
- Update to version 1.1.0g on AIX 6.1
- Add link from lib/lib*.a to lib64/lib*.a and deliver them.

* Thu May 30 2017 Tony Reix <tony.reix@atos.net> - 1.1.0f-1
- Update to version 1.1.0f on AIX 6.1

* Thu Feb 16 2017 Tony Reix <tony.reix@atos.net> - 1.1.0e-1
- Update to version 1.1.0e on AIX 6.1

* Fri Jan 27 2017 Tony Reix <tony.reix@atos.net> - 1.1.0d-1
- Update to version 1.1.0d on AIX 6.1

* Thu Jan 20 2017 Tony Reix <tony.reix@atos.net> - 1.1.0c-3
- Change: ln -s libcrypto.so.%{opensslversion} libcrypto.so
      to: ln -s libcrypto.so.1.0.2             libcrypto.so
  For 32 & 64bit, for sure.

* Thu Nov 24 2016 Tony Reix <tony.reix@atos.net> - 1.1.0c-2
- Big change:
  strip -e ALL .so files but the libssl.so.1.0.2 in libssl.a & libcrypto.a !!

* Thu Nov 16 2016 Tony Reix <tony.reix@atos.net> - 1.1.0c-1
- Update to version 1.1.0c on AIX 6.1
- Remove -bexpall
