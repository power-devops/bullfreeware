Summary: Ganglia Monitor daemon module ibmfc
Name: ganglia-mod_ibmfc
Version: 3.4.0
...
Patch2: ganglia-%{version}-%{release}-mod_ibmfc.patch
...
This gmond module provides Fibre Channel statistics for individual Fibre
Channel adapters on AIX.
...
%patch0
%patch1
# apply the patch for the mod_ibmfc module
%patch2
...
cp gmond/modules/ibmfc/.libs/modibmfc.so ${RPM_BUILD_ROOT}%{_libdir}/ganglia
...
%files
%defattr(-,root,system)
%{_libdir}/ganglia/modibmfc.so
%config(noreplace) %{conf_dir}/conf.d/ibmfc.conf
...
- changed method to detect all Fibre Channel adapters which are in
  state 'Available'
