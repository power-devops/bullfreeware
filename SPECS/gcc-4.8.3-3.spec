...
Source1: ftp://ftp.gnu.org/gnu/%{name}/%{name}-%{version}/%{name}-%{version}.tar.bz2.sig
Patch0: %{name}-%{version}-aix.patch
Patch1: %{name}-%{version}-aix51.patch
Patch2: %{name}-%{version}-aix52.patch
...
BuildRequires: bash, sed, automake, autoconf, make, tar, gcc
...
../%{name}-%{version}/configure \
	CC="/home/gnu/gcc482/bin/gcc  -std=gnu99" \
	CXX="/home/gnu/gcc482/bin/g++" \
	--with-gmp=/home/gnu/gcc482 \
	--with-ppl=/home/gnu/gcc482 \
	--with-mpfr=/home/gnu/gcc482 \
	--with-mpc=/home/gnu/gcc482 \
 	--enable-languages=c,c++,fortran \
	CFLAGS="-O2 -mpowerpc -mno-mfcrf -mtune=power6" \
	CXXFLAGS="-O2 -mpowerpc -mno-mfcrf -mtune=power6 " \
	CPPFLAGS="-fexceptions -I/home/gnu/gcc482/include" \
	LDFLAGS="-L/home/gnu/gcc482/lib -Wl,-bbigtoc -Wl,-bmaxdata:0x80000000" \
	--prefix=%{_prefix} \
	--mandir=%{_mandir} \
	--infodir=%{_infodir}
...
(
    ...
    [ -d ${RPM_BUILD_ROOT}%{_libdir64} ] || mkdir -p ${RPM_BUILD_ROOT}%{_libdir64}
    cd ${RPM_BUILD_ROOT}%{_libdir64}
    ln -sf ../lib/libatomic.a .
    ln -sf ../lib/libgcc_s.a .
    ln -sf ../lib/libgfortran.a .
    ln -sf ../lib/libgomp.a .
    ln -sf ../lib/libstdc++.a .
    ...
    [ -d ${RPM_BUILD_ROOT}%{_libdir}/pthread ] || mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/pthread
    ...
    [ -d ${RPM_BUILD_ROOT}%{_libdir}/pthread/ppc64 ] || mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/pthread/ppc64
    ...
    [ -d ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version} ] || mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}
    cd ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}
    ln -sf ../../../../lib/libatomic.a .
    ln -sf ../../../../lib/libgcc_s.a .
    ln -sf ../../../../lib/libgfortran.a .
    ln -sf ../../../../lib/libgomp.a .
    ln -sf ../../../../lib/libstdc++.a .
    ln -sf ../../../../lib/libstdc++.a-gdb.py .
    ln -sf ../../../../lib/libstdc++.la .
    ln -sf ../../../../lib/libsupc++.a .
    ln -sf ../../../../lib/libsupc++.la .
    ln -sf ../../../../lib/libgomp.la .
    ln -sf ../../../../lib/libgomp.spec .
    ln -sf ../../../../lib/libgfortran.la .
    ln -sf ../../../../lib/libgfortran.spec .

    cd ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}/include
    ln -sf ../../../../../include/c++ .

    [ -d ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}/pthread ] || mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}/pthread
    cd ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}/pthread
    ln -sf ../../../../../lib/libatomic.a .
    ln -sf ../../../../../lib/libgcc_s.a .
    ln -sf ../../../../../lib/libgfortran.a .
    ln -sf ../../../../../lib/libgomp.a .
    ...
    [ -d ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}/ppc64 ] || mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}/ppc64
    ...
    [ -d ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}/pthread/ppc64 ] || mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/gcc/%{buildhost}/%{version}/pthread/ppc64
...
%changelog
* Thu Feb 12 2015 BULL Gerard Visiedo <gerard.visiedo@bull.net>  4.8.3-3
- Add some over missed files in differents packages

* Mon Feb 09 2015 BULL Gerard Visiedo <gerard.visiedo@bull.net>  4.8.3-2
- Add missed files into libstdc++-devel package

* Fri Jan 16 2015 BULL Gerard Visiedo <gerard.visiedo@bull.net>  4.8.3-1
- Update to version 4.8.3 - languages C and C++ and fortran.

* Mon Dec 01 2013 BULL Gerard Visiedo <gerard.visiedo@bull.net>  4.8.1-2
- Add explicit libgcc preriquisite into package libstdc++

* Fri Oct 18 2013 BULL Gerard Visiedo <gerard.visiedo@bull.net>  4.8.1-1
- Update to version 4.8.1 - languages C and C++ and fortran.
- Built with XLC V12 (vacpp12PtfJun13-aix61-aix71).
