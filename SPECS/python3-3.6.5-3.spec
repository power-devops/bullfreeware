...
%define BASE_VERSION 3.6
#%define SHORT_VERSION 36
...
Source4: ChangePrimaryToPython2.sh
Source5: ChangePrimaryToPython3.sh
...
# Then patch it
# Patch7 and patch12 no more useful with Python 3.6.1
#%patch7 -p1 -b .fileio
#%patch12 -p1 -b .compat
...
export CXX__="/opt/freeware/bin/gcc"
export FLAG32="-maix32"
export FLAG64="-maix64 -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE -D_LARGE_FILES"
...
# Install the Change Primary Python Version shell scripts
...
%pre
# Test if there are different versions of Python installed
if eval "rpm -q --whatprovides /opt/freeware/bin/python"
then
  echo ""
  echo "If there are several versions of Python2 and Python3 installed,
  the following files should be renamed before installing this version."
  echo ""
  echo "Files concerned are in /opt/freeware/bin :
  python, python_64, pydoc, pydoc_64, python-config, \
  2to3, 2to3_64, idle and idle_64"
  echo ""
  echo "An existing Primary Python 2 Version can be moved before
  installing this version, if required, using a shell script similar to
  /opt/freeware/bin/ChangePrimaryToPython3.sh"
  echo ""
  echo "The Primary Python version can be chosen by using a shell script
  similar to
  /opt/freeware/bin/ChangePrimaryToPython2.sh    and
  /opt/freeware/bin/ChangePrimaryToPython3.sh"
  echo ""
  echo "To ignore this preinstall script and to ignore file conflicts
  between packages the following options are required
  --noscripts --replacefiles"
...
%post
  echo ""
  echo "If there are several versions of Python2 and Python3 installed,
  this newly installed version of Python3 will be the Primary."
  echo ""
  echo "The Primary Python Version can be chosen by using a shell script
  similar to
  /opt/freeware/bin/ChangePrimaryToPython2.sh    and
...
%preun
# Test if multiple versions installed and the Primary Python Version is 2
rpm -q --whatprovides /opt/freeware/bin/python
if /opt/freeware/bin/python --version 2>&1 | grep  'Python 2'
then
  echo ""
  echo "This uninstall concerns Python3, but the current Primary version of
  Python is 2. There are several versions of Python (2 abd 3) installed
  and the conflicting files (symbolic links) of Python 2 may be lost."
  echo ""
  echo "The Primary Python Version should be switched to 3 before uninstall
  ...
  2to3, 2to3_64, idle and idle_64"
  echo ""
  echo "A possible previous Primary Python 2 Version can be reinstated, if
  required, using a shell script similar to
  /opt/freeware/bin/ChangePrimaryToPython2.sh"
  echo ""
  echo "To ignore this preinstall script and to ignore file conflicts
  between packages the following options are required
  --noscripts --nodeps"
...
# Test if there are different versions of Python installed
  rpm -q --whatprovides /opt/freeware/bin/python
  echo ""
  echo "If the current uninstall concerns the Primary verson of Python
  and there are several versions of Python2 or Python3 installed,
  the following files should be reinstated after uninstalling this version."
  echo ""
  echo "Files concerned are in /opt/freeware/bin : 
  python, python_64, pydoc, pydoc_64, python-config, \
  2to3, 2to3_64, idle and idle_64"
...
%{_bindir}/ChangePrimaryToPython2.sh
%{_bindir}/ChangePrimaryToPython3.sh
%{_mandir}/*/*
/usr/bin/pydoc*
/usr/bin/python
/usr/bin/python_32
...
%{pylibdir64}/config-%{LDVERSION_optimized}/Makefile
...
* Thu Mar 26 2018 Daniele Silvestre <daniele.silvestre@atos.net> - 3.6.4-1
- Update to version 3.6.4
- Fix _tkinter.so delivery for 64bit version
- tkinter package always built

* Wed Aug 1 2017 Daniele Silvestre <daniele.silvestre@atos.net> - 3.6.2-1
