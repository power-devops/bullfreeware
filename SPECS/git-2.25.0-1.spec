...
Name: 		git
Version: 	2.25.0
Release: 	%{base_release}%{?without_ibm_SSL:opensourcessl}
Summary:  	Core git tools
License: 	GPLv2
Group: 		Development/Tools
URL: 		http://kernel.org/pub/software/scm/git/
Source0: 	http://kernel.org/pub/software/scm/git/%{name}-%{version}.tar.xz
Source1: 	http://kernel.org/pub/software/scm/git/%{name}-%{version}.tar.sign
...
Source8:	%{name}-%{version}-%{base_release}.build.log
...
Patch5:		%{name}-2.22.0-config-SHELL-and-AR-v2.patch
...
#     %if %{with ibm_SSL}
# 	# Force the use of /usr/lib/libssl.a and /usr/lib/libcrypto.a
# 	${SED} -i "s|-lssl|-L/usr/lib -lssl|g" config.mak.autogen
# 	${SED} -i "s|-lcrypto|-L/usr/lib -lcrypto|g" config.mak.autogen
# 	%endif
