...
Source0: ftp://ftp.gnu.org/gnu/%{base_name}/%{base_name}-%{version}/%{base_name}-%{gcc_version}.tar.gz
# Used for snapshots:
...
Patch30: gcc-10-aix-Add-GCC64-configuration-and-FAT-target-libraries.patch
...
Patch43: gcc-10-aix-Add-FAT-library-support-for-libffi.patch
...
export MACRO_LIST=%{gcclibdir}/install-tools/macro_list
...
# systimits.h if it has been modified. Otherwize, it provides its own
# syslimits.h.
if [ -f $TMPDIR/limits.h ]; then
	mv $TMPDIR/limits.h $TMPDIR/syslimits.h
else
	cp %{gcclibdir}/install-tools/gsyslimits.h $TMPDIR/syslimits.h
fi
cp %{gcclibdir}/install-tools/include/limits.h $TMPDIR

# Copy README from gcc include-fixed directory.
# TODO: Check for every new gcc versions as new files can be provided
# by gcc itself.
cp %{gcclibdir}/install-tools/include/README $TMPDIR
