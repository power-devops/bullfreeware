...
#WARNING: This rpm doesn't work porperly with rrdtools rpm package
...
${CC} -q64 -qmkshrobj lib%{name}.a -o lib%{name}-5.2.so -bE:lib%{name}.exp -lm
rm -f lib%{name}.a
/usr/bin/ar -X64 -rc lib%{name}.a lib%{name}-5.2.so
...
${CC} -q32 -qmkshrobj lib%{name}.a -o lib%{name}-5.2.so -bE:lib%{name}.exp -lm
rm -f lib%{name}.a
/usr/bin/ar -X32 -rc lib%{name}.a lib%{name}-5.2.so
rm -f %{name}.exp lua luac
...
mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/%{name}/5.2
mkdir -p ${RPM_BUILD_ROOT}%{_datadir}/%{name}/5.2
...
%changelog
* Mon Mar 26 2012 Gerard Visiedo <gerard.visiedo@bull.net> - 5.2.0-1
