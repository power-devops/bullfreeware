...
Source4: mkvirtpkg.rpm-4.9.1.3-3
Patch0: rpm-%{version}-aix.patch
Patch1: rpm-%{version}-rpmrc2.patch
...
do
   ...
   # Adding some fake commands block into the spec file
   echo '
#%prep
#%build
#%install
...
define _srcrpmdir /tmp/tmprpm/rpmdir/SRPMS' | sed "s/^/%/" >> ${VIRTUALSDIR}/mkvirtpkg.${pac}
   ...
   do
       ...
       then
         ...
         # Should check if ${item} exists and if not, add %ghost (dbus)
         # Also, must protect rpm against file names with space characters
         if [ ! -e "${item}" ]
         then
           echo \"${item}\" | sed "s!^!%ghost !" >> ${VIRTUALSDIR}/mkvirtpkg.${pac}
         else
           echo ${item} | grep -q "/doc/" ; status=$?
           [ $status -eq 0 ] \
...
then
   ...
   do
     # If RPM version 4 already installed, option -ba is for rpmbuild
     if test -x /usr/bin/rpmbuild
     then
      /usr/bin/rpmbuild -ba ${f} > ${TMPDIR}/${f}.$$.out 2>&1
     else
...
export VIRTUALSDIR=${TMPDIR}/virtuals
sleep 1
cp /tmp/mkvirtpkg.rpm-4.9.1.3-3 ${VIRTUALSDIR}
# If RPM version 4 already installed, option -ba is for rpmbuild
if test -x /usr/bin/rpmbuild
then
/usr/bin/rpmbuild -ba /tmp/mkvirtpkg.rpm-4.9.1.3-3 >${TMPDIR}/mkvirtpkg.rpm-4.9.1.3-3.$$.out 2>&1
else
/usr/bin/rpm -ba /tmp/mkvirtpkg.rpm-4.9.1.3-3 >${TMPDIR}/mkvirtpkg.rpm-4.9.1.3-3.$$.out 2>&1
fi
slibclean
...
%changelog
* Tue Oct 20 2015 Michael Wilson <michael.wilson@bull.net> 4.9.1.3-3
- Fix dependencies, update patch for AIX 7.2 and repackage on AIX 61e
