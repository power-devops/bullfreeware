...
%define BASE_VERSION %{major_version}.%{minor_version}
...
Source1: pyconfig.h
Source2: https://www.python.org/ftp/python/doc/%{version}/python-%{version}-docs-text.tar.bz2
...
BuildRequires: expat-devel >= 1.18.1-5
BuildRequires: gdbm-devel >= 2.2.10
...
# Really required without python3-tkinter?
BuildRequires: tcl >= 8.6.10-3
BuildRequires: tk >= 8.6.10-3
...
BuildRequires: tk-devel  >= 8.6.10-3
Requires: tcl >= 8.6.10-3
Requires: tk  >= 8.6.10-3
Requires: %{name} = %{version}-%{release}
...
build_python()
{
	...
	--host=%{buildhost} --target=%{buildhost} --build=%{buildhost} \
        ...
        --with-system-ffi \
        --with-system-expat \
        --with-tcltk-includes="-I %{_includedir}" \
        --with-tcltk-libs="-L $1 -ltcl -ltk" \
	%{WITH_WITHOUT_MALLOC} \
        %{WITH_WITHOUT_GOTOS}
...
# We use CreateExportList only with XLC.
# Both way (CreateExportList and use .o) should be OK with both compiler.

%if %{with gcc_compiler}
    # GCC
    mkdir tmp_obj
    cd tmp_obj
    ${AR} -x ../libpython%{VERSION_LIB}.a
    cd ..
    ${CC} ${CFLAGS} -shared tmp_obj/*.o -o libpython%{VERSION_LIB}.so  -lm -lpthreads -lintl -Wl,-blibpath:$1:/opt/freeware/lib:/usr/lib:/lib
...
%exclude %{_libdir}/python3.8/test/imghdrdata/python.exr
%exclude %{_libdir64}/python3.8/test/imghdrdata/python.exr
...
#################################
%changelog
* Mon Jan 04 2021 Clément Chigot <clement.chigot@atos.net> - 3.8.6-2
- Update minimal versions for dependencies

* Thu Nov 26 2020 Étienne Guesnet <etienne.guesnet@atos.net> - 3.8.6-1
- New version 3.8.6

* Thu Nov 12 2020 Étienne Guesnet <etienne.guesnet@atos.net> - 3.8.5-4
- Update specfile for automated build
- Use new tcl / tk
- Stop using CreateExportList for GCC
- Stop providing some tests files that fail with 'file' command
  and cause rpm build failure
- Update license
- Bit-flag in both CFLAGS and CC.
