...
# GCC may be used to build a future (4.5.3) version
#
# samba-4.4.5 contains new security fixes and a number of code changes
# samba-4.4.9 contains new security fixes and a number of code changes
#
# Options have been included from 4.5.0 in the SPEC file to build/not build 
# Samba components for testing and cluster data base (ctdb vs tbd)
# Options are not included for the build of cluster data base (ctdb vs tbd)
#
# samba-4.5.1 contains new security fixes and a number of code changes
# samba-4.5.3 contains new security fixes and a number of code changes
# samba-4.5.10 contains new security fixes and a number of code changes
#
# samba-4.6.4 initial version 4.6 at same security fixe level as 4.5.10
# samba-4.6.7 at same Heimdal security fixe level as 4.5.13
#
# samba-4.6.11 at same CVE security fixe level as 4.5.15
#
# samba-4.6.14 at same CVE security fixe level as new port 4.7.6
# New port samba-4.7.6 at same CVE security fixe level as 4.6.14
#
# samba-4.7.6 initial version 4.7 at same security fixe level as 4.6.14
# samba-4.7.10 at same CVE security fixe level as 4.6.16
#
# New port samba-4.9.0 at same CVE security fixe level as 4.7.10
# samba-4.9.4  CVE security (4.9.3) and bug fixes
#
...
Source12: samba_winbind_idmap_syms.exp
...
Patch6: samba-4.9.4-Struc_bad_initialization.patch
...
# Export winbindd binary symbols for idmap/ad.so winbindd_ads.c/wb_dsgetdcname.c
Patch15: samba-4.6.11-winbindd_exports.patch

# EINVAL returned from pthread_cond_destroy() after fork() on AIX
# Patch16: samba-4.6.11-pthreadpool_cond_destroy.patch
# Patch supplied by Volker Lendecke <vl@samba.org>
# No longer required in 4.6.14
# Patch16: samba-4.6.11-PthreadCond.patch

# Patch17: samba-4.7.0-bind_dlz.patch

# Platform independent code errors - to be submitted to Samba.com ?
# Patch18: samba-4.7.6-tfork.patch
# Aggregated patch send for test by Stefan Metzmacher (Samba team member)
# Integrated in Samba 4.7.10
# Patch18: samba-4.7.6-Aggregated.patch

# Unfortunately, this part of Aggregated did not make it into the 4.7.10 source
# Integrated in Samba 4.9.0
# Patch19: samba-4.7.6-vfs_aixacl2.patch

# Fedora following patch for compile flags
# Removed in 4.9.4  Patch20: samba-4.9.0rc5-stack-protector.patch
...
# 2 New in 4.9 w.r.t. 4.7.10 - rpcgen is delivered in LPP bos.net.nfs.server
...
cp samba-%{version}/source3/script/mksmbpasswd.sh ${RPM_BUILD_ROOT}%{_bindir}/
...
cp samba-%{version}/packaging/printing/smbprint ${RPM_BUILD_ROOT}%{_bindir}/
...
cp samba-%{version}/examples/LDAP/%{name}.schema ${RPM_BUILD_ROOT}/etc/openldap/schema/%{name}.schema
chmod 0644 ${RPM_BUILD_ROOT}/etc/openldap/schema/%{name}.schema

# winbind krb5 locator
mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/krb5/plugins/libkrb5
cp samba-%{version}/bin/shared/winbind_krb5_locator.so ${RPM_BUILD_ROOT}%{_libdir}/krb5/plugins/libkrb5/winbind_krb5_locator.so
...
cp samba-%{version}/bin/shared/WINBIND.so ${RPM_BUILD_ROOT}%{_libdir}/security/WINBIND
...
#done
cp samba-%{version}/bin/default/source3/libsmb/smbclient.pc ${RPM_BUILD_ROOT}%{_libdir}/pkgconfig/ 
cp samba-%{version}/bin/default/source3/libnet/netapi.pc ${RPM_BUILD_ROOT}%{_libdir}/pkgconfig/ 
cp samba-%{version}/bin/default/nsswitch/libwbclient/wbclient.pc ${RPM_BUILD_ROOT}%{_libdir}/pkgconfig/

mkdir -p ${RPM_BUILD_ROOT}/etc/sysconfig
cp %{SOURCE4} ${RPM_BUILD_ROOT}/etc/sysconfig/%{name}
chmod 0644 ${RPM_BUILD_ROOT}/etc/sysconfig/%{name}

# cp -r samba-%{version}/source3/lib/netapi/examples/netdomjoin-gui ${RPM_BUILD_ROOT}%{_sbindir}/
# chmod 0755 ${RPM_BUILD_ROOT}%{_sbindir}/netdomjoin-gui

# mkdir -p ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}
# cp samba-%{version}/source3/lib/netapi/examples/netdomjoin-gui/%{name}.ico ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/%{name}.ico
# chmod 0644 ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/%{name}.ico

# cp samba-%{version}/source3/lib/netapi/examples/netdomjoin-gui/logo.png ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/logo.png
# chmod 0644 ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/logo.png

# cp samba-%{version}/source3/lib/netapi/examples/netdomjoin-gui/logo-small.png ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/logo-small.png
...
%if %{with testsuite}
%check
(LIBPATH=${RPM_BUILD_ROOT}%{_libdir}/:${RPM_BUILD_ROOT}%{_libdir}/samba:$LIBPATH  TDB_NO_FSYNC=1 gmake test || true)
%endif
...
# CTDB not built %{_libdir}/samba/libctdb-event-client-samba4.so
...
# Removed  %{_libdir}/samba/libgpo-samba4.so
...
# Internal or independent TALLOC library/package (2.1.9)
...
# %{_libdir}/libnss-winbind.so*
# %{_libdir}/libnss-wrapper-winbind.so*
%{_libdir}/security/WINBIND
