...
Patch1: %{name}-CAN-2005-0100.patch
...
CONFOPTS="--prefix=%{_prefix} --libexecdir=%{_prefix}/lib --sharedstatedir=/var  --with-pop"

#Build binary with X support
[ -d build-withx ] && rm -rf build-withx
mkdir build-withx && cd build-withx
CFLAGS="$RPM_OPT_FLAGS" LDFLAGS=-s \
  ../configure ${CONFOPTS} --with-x-toolkit
make
cd ..
