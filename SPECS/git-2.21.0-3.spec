...
Source10:	git-2.21.0-SED.patch

Source99:	git-print-failed-test-output.sh

Patch0: 	%{name}-1.8.3.1-aix.patch
...
Patch5:		%{name}-2.21.0-fileno.patch
...
# 2 attempts for fixing access(, _OK) issues
# Listed here for saving them. Do not use for building RPMs
Patch81:	%{name}-2.21.0-access-root.patch
Patch82:	%{name}-2.21.0-access-root-ala-bash.patch
...
(find ${RPM_BUILD_ROOT}%{_libexecdir} -type fl | grep -vE "archimport|svn|cvs|email|gitk|git-gui|git-citool|git-daemon" | sed -e s@^${RPM_BUILD_ROOT}@@) >> bin-man-doc-files
...
(find ${RPM_BUILD_ROOT}/usr/bin -type fl | grep -vE "archimport|svn|cvs|email|gitk|git-gui|git-citool|git-daemon" | sed -e s@^${RPM_BUILD_ROOT}@@) >> bin-man-doc-files
