...
Patch26:        mariadb-10.5.4-skip_test.patch
# Compilation flags and avoid RPM configuration
Patch27:        mariadb-10.3.12-correct_flags.patch
...
# PRIu64 bug
Patch41:        mariadb-10.5.4-PRIu.patch
...
BuildRequires:    pcre-devel >= 8.43 pkg-config
...
pcre_system_version=`pcregrep --version | sed "s|^.*version ||" | sed "s| .*$||"`
...
then
  ...
  echo "Bundled PCRE will be used."
  PCRE="bundled"
else
  echo "System PCRE will be used if it has stack_guard"
  PCRE="auto"
  #rm -rf pcre
fi
...
# Run default tests
perl_64 ./mysql-test-run.pl %{common_testsuite_arguments} \
...
%if "%{spider}" != "NO"
# Third run for the SPIDER suites that fail with SCA (ssl self signed certificate)
...
rm ${RPM_BUILD_ROOT}/%{_bindir}/wsrep_sst_rsync_wan
