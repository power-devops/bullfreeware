...
Source1:    https://www.apache.org/dyn/closer.cgi/perl/mod_perl-2.0.11.tar.gz
Source3:    %{name}.aix.init
Source4:    %{name}-%{version}.conf.patch
Source5:    %{name}-ssl-2.4.X.conf.patch
Source6:    %{name}-ssl-2.4.X.conf64.patch
Source9:    %{name}-%{version}.configure.patch
Source10:   %{name}-%{version}.conf64.patch
...
cd 32bit
#export CFLAGS="-q32 -qchars=signed -D_LARGE_FILES -O2"
...
./configure \
    ...
    --with-apr-util=%{_bindir}/apu-1-config_32 \
    --with-ldap=yes \
    --enable-so \
    ...
    --enable-dav \
    --with-lua=%{_prefix} \
    --with-pcre=%{_bindir}/pcre-config_64 \
    --disable-auth-digest \
    --disable-distcache \
    --disable-imagemap \
    --enable-modules=all \
...
cd ../64bit
#export CFLAGS="-q64 -qchars=signed -O2"
...
./configure \
    ...
    --sysconfdir=%{_sysconfdir}/httpd/conf \
    --includedir=%{_includedir}/httpd \
    --libexecdir=%{_libdir}64/httpd/modules \
    --localstatedir=%{localstatedir} \
    --datadir=%{contentdir} \
    --enable-mpms-shared=all \
    --with-apr=%{_bindir}/apr-1-config_64 \
    --with-apr-util=%{_bindir}/apu-1-config_64 \
    --with-ldap=yes \
    --enable-so \
    --enable-suexec --with-suexec \
    --with-suexec-caller=%{suexec_caller} \
...
MP_APXS=./apxs /opt/freeware/bin/perl_64 Makefile.PL
...
# sed -i "s|/opt/freeware/lib/|/opt/freeware/lib64/|g" t/conf/httpd.conf
if [ "`id -u`" -eq "0" ]; then
...
# cp ${RPM_BUILD_ROOT}/%{_libdir}/httpd/build/config_vars.mk .
# 
# # Use RPM_BUILD_ROOT, except for apr
# sed -i "s|/opt|${RPM_BUILD_ROOT}/opt|g"                                                     config_vars.mk
# sed -i "s|${RPM_BUILD_ROOT}/opt/freeware/bin/apr-1-config|/opt/freeware/bin/apr-1-config|g" config_vars.mk
# sed -i "s|${RPM_BUILD_ROOT}/opt/freeware/bin/apu-1-config|/opt/freeware/bin/apu-1-config|g" config_vars.mk
# sed -i 's|get_config_vars("$installbuilddir/|get_config_vars("./|' apxs
# 
# MP_APXS=./apxs /opt/freeware/bin/perl_32 Makefile.PL
# 
# # Perl used is 64 bit
...
# gmake
# 
# # No need to create apache id
# chown -R apache:apache .
# 
# # We cannot change this conf location, so we copy original before alter it
# cp ${RPM_BUILD_ROOT}/%{_sysconfdir}/httpd/conf/httpd.conf ${RPM_BUILD_ROOT}/%{_sysconfdir}/httpd/conf/httpd.conf.orig
# # We want the 32 bit conf file
...
# # Restore right files
# mv ${RPM_BUILD_ROOT}/%{_sysconfdir}/httpd/conf/httpd.conf.orig ${RPM_BUILD_ROOT}/%{_sysconfdir}/httpd/conf/httpd.conf
# mv ${RPM_BUILD_ROOT}/%{_prefix}/sbin/httpd_orig ${RPM_BUILD_ROOT}/%{_prefix}/sbin/httpd
# cd ..
%endif
