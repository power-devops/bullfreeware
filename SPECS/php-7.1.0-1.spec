...
Patch2: %{name}-7.0.7-aix-setrlimit.patch
Patch3: %{name}-7.0.7-aix-strfmon.patch
Patch4: %{name}-7.0.10-aix-tests.patch
Patch5: %{name}-7.1.0-aix-trailing-slash.patch
...
# export QUOTE=\'
# sed -e 's;"s/'$QUOTE'$var=$val'$QUOTE'//";"s|'$QUOTE'$var=$val'$QUOTE'||";'    <  configure > configure.tmp
# cp configure.tmp  configure; rm -f  configure.tmp
...
# Problems with nm and lib64
# nm : 'ext/date/.libs/php_date.o': No such file
# /opt/freeware/lib/libcrypto.so: XCOFF32 object files are not allowed in 64-bit mode.
# 
# We decided to not develop the 64-bit version:
# The IBM apache lpp is only 32-bit
# 
# cp -pr 32bit 64bit
...
# Do not build 64 bits versions (Problems)
# build on 64bit mode
# cd 64bit
