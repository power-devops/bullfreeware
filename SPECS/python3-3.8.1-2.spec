...
build_python()
{
    ...
    gmake %{?_smp_mflags} libpython%{VERSION_LIB}.a
    
	$XLCCPATH/CreateExportList -X${OBJECT_MODE} libpython.exp libpython%{VERSION_LIB}.a
%if %{with gcc_compiler}
    # GCC
    ${CC} -shared    libpython%{VERSION_LIB}.a -o libpython%{VERSION_LIB}.so -Wl,-bE:libpython.exp -lm -lpthreads -lintl -Wl,-blibpath:$1:/opt/freeware/lib:/usr/lib:/lib
%else
    # xlc
${CC} -qmkshrobj libpython%{VERSION_LIB}.a -o libpython%{VERSION_LIB}.so     -bE:libpython.exp -lm -L/opt/freeware/lib -lintl -blibpath:$1:/opt/freeware/lib:/usr/lib:/lib
%endif
    rm -f libpython.exp
