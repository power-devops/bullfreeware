...
Patch:          snappy-1.1.3-static-fct.patch
...
%prep
export PATH=/opt/freeware/bin:/usr/bin:/bin
%setup
%patch0 -p1 -b .static
...
export LDFLAGS="-L`pwd` -L`pwd`/.libs -L%{_libdir64} -L%{_libdir} -L/usr/lib64 -L/usr/lib -Wl,-brtl"
...
export LDFLAGS="-L`pwd` -L`pwd`/.libs -L%{_libdir} -L/usr/lib -Wl,-brtl"
...
${AR} -q .libs/libsnappy.a  .libs/libsnappy.so.1.3.0 .libs64/libsnappy.so.1.3.0
...
%makeinstall
/opt/freeware/bin/find %{buildroot} -name \*.la -delete -print
...
%changelog
* Thu May 03 2018 Tony Reix <tony.reix@atos.net>  1.1.3-1
- New version 1.1.3
