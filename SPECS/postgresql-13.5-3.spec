# compiler by default: gcc, default_bits 64
...
Source2: https://www.postgresql.org/files/documentation/pdf/%{major_version}/postgresql-%{major_version}-US.pdf
...
Source13: %{name}-%{major_version}.resultats.ksh
...
#Patch0: %{name}-13.4-aix.patch

#	Patch1: %{name}-13.4-LARGE_FILES.patch
# 	Replaced by -D_LARGE_FILES

Patch2: %{name}-13.4-aix_TestLib.pm.patch
...
#	Patch3: %{name}-13.4-32bit-XLCv13-O2-inline-pragma-O0.patch
# Used for removing the remove of inline for AIX/XLC/32bits
#	Patch4: %{name}-13.4-32bit-XLC-O2-inline-NO-PG_FORCE_DISABLE_INLINE.patch
#	Patches 3 & 4 replaced by:
Patch3: %{name}-13.4-AIX-XLC-32-O2-inlining.patch

# Used for enabling tracing with tprof
Patch6: %{name}-13.4-ForTprof.patch
...
#	Patch7: %{name}-13.4-NoKill.patch
# Replaced by:
Patch7: %{name}-13.4-AIX-spinlocks.patch

# No more useful. Seems included in version 10.2
#Patch5: %{name}-13.4-sigpipe-tap-v1.patch

# Required for zic.c with XLC -O3 -q32
#	Patch8: %{name}-13.4-32bit-XLCv1317-O3-zic-pragma-O0.patch
Patch8: %{name}-13.4-AIX-XLC-32-O3-zic.patch
...
#Patch9:  %{name}-13.4-AIX-SysV-SharedMemory-LargePagesV2.patch

# New patches made by community for AIX
# v12.4 Recreating and renaming, due to fuzz=0
Patch91:  %{name}-13.5-AIX-Add-huge-page-support.patch
Patch92:  %{name}-13.4-AIX-Add-huge-page-support-shared_memory_type_sysv.patch

# Patch for using exports.txt for building lib*.exp for libpq.a and Co, when the file does exist
# Already applied now in v13.5 PostgreSQL source base: no more useful
#	Patch10:  %{name}-13.4-exports.txt.patch


BuildRequires: bison, flex, gawk, make
#BuildRequires: jade
BuildRequires: bzip2 >= 1.0.2
BuildRequires: perl(perl) >= 5.34
...
# configure :
#	checking for PYTHON
#	configure:9653: result: /opt/freeware/bin/python
...
%if %plpython
%package -n python%{python_version}-postgresql
...
%description -n python%{python_version}-postgresql
PostgreSQL is an advanced Object-Relational database management
system.  The postgresql-plpython package contains the PL/Python
procedural language for the backend.

%package -n python3-postgresql
Summary: The Python procedural language for PostgreSQL
Group: Applications/Databases
Requires: python(abi) = %{python_version}
Requires: python%{python_version}-postgresql = %{version}-%{release}

%description -n python3-postgresql
PostgreSQL is an advanced Object-Relational database management
system.  The postgresql-plpython package contains the PL/Python
procedural language for the backend.
%endif
...
# PERLVERSION=`/usr/bin/rpm - q a | grep perl-`
...
# Check that pg_encoding_to_char() symbol does exist (issue when building libpq.exp)
/usr/bin/nm -X64 src/interfaces/libpq/libpq.a | grep pg_encoding_to_char
...
# Check that pg_encoding_to_char() symbol does exist (issue when building libpq.exp)
/usr/bin/nm -X32 src/interfaces/libpq/libpq.a | grep pg_encoding_to_char
...
my_find_lang ecpg-%{major_version}
cat ecpg-%{major_version}.lang > devel.lst
my_find_lang ecpglib6-%{major_version}
cat ecpglib6-%{major_version}.lang > libs.lst
my_find_lang initdb-%{major_version}
cat initdb-%{major_version}.lang > server.lst
my_find_lang libpq5-%{major_version}
cat libpq5-%{major_version}.lang >> libs.lst
my_find_lang pg_basebackup-%{major_version}
cat pg_basebackup-%{major_version}.lang >>server.lst
my_find_lang pg_controldata-%{major_version}
cat pg_controldata-%{major_version}.lang >> server.lst
my_find_lang pg_ctl-%{major_version}
cat pg_ctl-%{major_version}.lang >> server.lst
my_find_lang pg_config-%{major_version}
cat pg_config-%{major_version}.lang > main.lst
my_find_lang pg_dump-%{major_version}
cat pg_dump-%{major_version}.lang >> main.lst
my_find_lang pg_resetwal-%{major_version}
cat pg_resetwal-%{major_version}.lang >> server.lst
my_find_lang pgscripts-%{major_version}
cat pgscripts-%{major_version}.lang >> main.lst

%if %plperl
my_find_lang plperl-%{major_version}
cat plperl-%{major_version}.lang > plperl.lst
%endif

my_find_lang plpgsql-%{major_version}
cat plpgsql-%{major_version}.lang >> server.lst

%if %plpython
my_find_lang plpython-%{major_version}
cat plpython-%{major_version}.lang > plpython.lst
%endif

%if %pltcl
my_find_lang pltcl-%{major_version}
cat pltcl-%{major_version}.lang > pltcl.lst
%endif

my_find_lang postgres-%{major_version}
cat postgres-%{major_version}.lang >> server.lst
my_find_lang psql-%{major_version}
cat psql-%{major_version}.lang >> main.lst
...
%if %plperl
%if %nls
%files -n perl%{perl_version}-postgresql -f 32bit/plperl.lst
%defattr(-,root,system)
%else
%files -n perl%{perl_version}-postgresql
%defattr(-,root,system)
%endif
...
%if %plpython
%if %nls
%files -n python%{python_version}-postgresql -f 32bit/plpython.lst
%defattr(-,root,system)
%else
%files -n python%{python_version}-postgresql
%defattr(-,root,system)
%endif
...
%changelog
* Wed Dec 1 2021 Etienne Guesnet <etienne.guesnet@atos.net> - 13.5-3
- Add python metapckage.
- Remove plpython for 32 bits

* Wed Nov 24 2021 Bullfreeware Continous Integration <bullfreeware@atos.net> - 13.5-2
- Rebuild 13.5

* Tue Nov 23 2021 Tony Reix <tony.reix@atos.net> - 13.5-1
- Adapt patches for v13.5

* Tue Nov 02 2021 Étienne Guesnet <etienne.guesnet@atos.net> - 13.4-3
- Mass rebuild for new version of perl.
- Add perl metapackage.
- Move hstore_plperl.so to perl subpackage.

* Wed Sep 01 2021 Tony Reix <tony.reix@atos.net> - 13.4-2
- Provide a clean fix for libpq.exp issue (missing symbols)

* Wed Aug 25 2021 Tony Reix <tony.reix@atos.net> - 13.4-1
- Move to v13.4
- Merge export list files

* Thu Dec 03 2020 Tony Reix <tony.reix@atos.net> - 13.1-1
- Move to v13

* Mon Nov 30 2020 Tony Reix <tony.reix@atos.net> - 12.5-1
- Move to v12.5
