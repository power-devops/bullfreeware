...
# The multilib_capable variable not defined for install & mysql_config_multilib
%global multilib_capable false
...
# 
# With %bcond_with : This variable is created and set false. Thus:
#	 %{with VAR} 		: is false
#	 %{?with_VAR: ACTION}   : ACTION is not executed
# It can be set to true  by means of: --with    debug 
# It can be set to false by means of: --without debug 
%bcond_with debug
...
Patch2:           %{pkgnamepatch}-s390-tsc-8.0.17-2.patch
...
Patch10032:	mysql-8.0.16-NO-gc-sections.patch
Patch1004:	mysql-8.0.16-tell.patch
Patch1005:	mysql-8.0.17-export_link_flags.patch
Patch1006:	mysql-8.0.17-thread.patch
#	Patch1007:	mysql-8.0.16-template-with-C-linkage.patch
Patch1008:	mysql-8.0.16-longlong-uint64.patch
Patch1009:	mysql-8.0.17-undefined-berok.patch
Patch1010:	mysql-8.0.16-rpcgen-C.patch
Patch1011:	mysql-8.0.16-sockio.h.patch
Patch1012:	mysql-8.0.16-sock_probe_ix.patch
Patch1013:	mysql-8.0.16-getopt.patch
Patch1014:	mysql-8.0.16-thread_init_end.patch

# TODO: TEMPORAIRE !!!!!!!! Remettre plus tard !
Patch1015:	mysql-8.0.17-group_replication.patch

Patch1016:	mysql-8.0.16-perl.patch

Patch1017:	mysql-8.0.16-tests.patch
Patch1018:	mysql-8.0.16-thread_local.patch
Patch1019:	mysql-8.0.16-sql_yacc-R_TOCL.patch
Patch1020:	mysql-8.0.16-perl2.patch

# After build: (Still useful after extending -perl.patch ??)
# Deja fait
#	Patch1021:	mysql-8.0.16-perl3.patch

Patch1022:	mysql-8.0.17-plugins.patch
...
# Fixes auth_sec.keyring_udf
# Not useful with 8.0.17 due to previous patches
#	Patch1025:	mysql-8.0.16-keyring_udf-current_thd.patch

# For release build
Patch1026:	mysql-8.0.17-Release.patch
...
# Patch to be done by hand for now ONCE for cmake:
#		mysql-8.0.16-cmake-isystem.patch
...
%global __provides_exclude_from ^(%{_datadir}/(mysql|mysql-test)/.*|%{_libdir64}/mysql/plugin/.*\\.so)$
#	%else
#	%filter_from_requires /perl(\(hostnames\|lib::mtr\|lib::v1\|mtr_\|My::\)/d
#	%filter_provides_in -P (%{_datadir}/(mysql|mysql-test)/.*|%{_libdir64}/mysql/plugin/.*\.so)
#	%filter_setup
%endif
...
%if %{with big_test}
SO=`/usr/bin/df -k /opt | awk '{if(NR==2)print $3}'`
echo "Disk space available for testing in big-test MySQL must be >= 100 Gio"
if [ "$SO" -lt 100000000 ]
then
    echo "Not enough disk space on /opt !"
    exit 1
...
echo "Check -isystem is not used for AIX with gcc !!!!"
grep CMAKE_INCLUDE_SYSTEM_FLAG /opt/freeware/share/cmake-3.14/Modules/Compiler/GNU.cmake
...
# INUTILE !!!!!!!??
#	export CMAKE_C_IMPLICIT_LINK_LIBRARIES="/opt/freeware/lib/gcc/powerpc-ibm-aix7.2.0.0/8.3.0/pthread/ppc64/libgcc.a;c"
#	export CMAKE_C_IMPLICIT_LINK_DIRECTORIES="/opt/freeware/lib/pthread/ppc64;/opt/freeware/lib/gcc/powerpc-ibm-aix7.2.0.0/8.3.0/pthread/ppc64;/opt/freeware/lib/gcc/powerpc-ibm-aix7.2.0.0/8.3.0/pthread;/opt/freeware/lib64;/opt/freeware/lib;TONY"
#	export CMAKE_CXX_IMPLICIT_LINK_LIBRARIES="stdc++;m;gcc_s;/opt/freeware/lib/gcc/powerpc-ibm-aix7.2.0.0/8.3.0/pthread/ppc64/libgcc.a;pthreads;c;gcc_s"
#	export CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES="/opt/freeware/lib/pthread/ppc64;/opt/freeware/lib/gcc/powerpc-ibm-aix7.2.0.0/8.3.0/pthread/ppc64;/opt/freeware/lib/gcc/powerpc-ibm-aix7.2.0.0/8.3.0/pthread;/opt/freeware/lib/lib64;/opt/freeware/lib;TONY"
...
# TODO : remettre le PLUGIN_group_replication=YES !
...
%{?with_debug: -DMYSQL_MAINTAINER_MODE=0} \
         -DTMPDIR=/var/tmp \
	 -DWITH_MYSQLD_LDFLAGS="-DTONYREIX2"

# Used by CMAKE ??
#	 -DWITH_MYSQLD_LDFLAGS="-Wl,-bbigtoc"
...
# Useful for listing the variables # It does not build
#cmake .. -LAH
...
# TO be done on AIX???
#rm %{buildroot}%{_libdir64}/mysql/*.a
...
# TODO:
echo "STRIPPER  LE  .so liblibmysql.so de libmysqlclient.a ?"
...
# --debug
# --manual-gdb
# Removed ?  --mysqld=--binlog-format=mixed
...
#	DEBUG=" --mark-progress "
#       DEBUG="$DEBUG --debug "
#       DEBUG="$DEBUG --debug --manual-gdb "
#       DEBUG="$DEBUG --manual-gdb "
#	DEBUG="--boot-gdb --client-gdb"
#	DEBUG="--boot-gdb --gdb"
  ...
  ulimit -m unlimited
  ulimit -d unlimited
#  ulimit -f unlimited	 # Not possible
...
(
  ...
  # Use: --suite=all !!
  # Change for : parallel = 8 instead of 1
  (./mtr \
     --warnings \
     --verbose \
