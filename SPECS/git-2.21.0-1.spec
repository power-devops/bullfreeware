...
# Not enough. Requires also: export TAR=...........
Patch6:		%{name}-2.21.0-TAR.patch
Patch7:		%{name}-2.21.0-SIGCHLD-handler-v2.patch
# Still issues...
#Patch8:		%{name}-2.21.0-access-root.patch
...
(
  ...
  do
    ...
    if [ -f $f ]
    then
      ( (grep "#\! /bin/sh" $f 2> /dev/null ; echo $? > /tmp/git-sh-$$) || true )
      RES=`cat /tmp/git-sh-$$`
      if [ $RES -eq 0 ]
      then
      ...
      ( (grep "#\!/usr/bin/sh" $f 2> /dev/null ; echo $? > /tmp/git-sh-$$) || true )
      RES=`cat /tmp/git-sh-$$`
      if [ $RES -eq 0 ]
      then
