...
Behavior of CMake on AIX has been changed to obey to AIX rules.
See /opt/freeware/share/cmake/readme-aix.txt for more informations.
...
%patch1 -p1  -b .NewAIXbehavior
%patch2 -p1  -b .NewAIXbehavior_tests
%patch3 -p1  -b .NewAIXbehavior_readme
...
%ifos aix6.1
export CFLAGS="-DSYSV -D_AIX -D_AIX61          -D_ALL_SOURCE -DFUNCPROTO=15 -O2" 
%endif
%ifos aix7.1 aix7.2
export CFLAGS="-DSYSV -D_AIX -D_AIX71 -D_AIX72 -D_ALL_SOURCE -DFUNCPROTO=15 -O2"
%endif
...
# Use -mcmodel=large instead of:  -Wl,-bbigtoc .
%ifos aix6.1
export LDFLAGS="-Wl,-bbigtoc"
%else
export CFLAGS="-mcmodel=large $CFLAGS"
export LDFLAGS=""
%endif
...
gmake --trace %{?_smp_mflags} VERBOSE=1 -j16

####### TEST #######
if [ "%{dotests}" == 1 ]
then
  if [ -e /lib/syscalls.exe ]
   then
    TEST_ARGS=""
   else
  ...
  rm -rf Tests_GCC ; mv Tests Tests_GCC
  ...
  (ARGS=$TEST_ARGS gmake -k test || true)
  rm -rf Tests_XLC ; mv Tests Tests_XLC
  ...
  export LDFLAGS="-L/opt/freeware/lib64 -Wl,-blibpath:/opt/freeware/lib64:/opt/freeware/lib:/usr/lib:/lib -lintl"
  cp -r Tests_orig Tests
  (ARGS=$TEST_ARGS gmake -k test || true)
...
%changelog
* Mon Mar 18 2019 Etienne Guesnet <etienne.guesnet.external@atos.net> - 3.14.1-1
