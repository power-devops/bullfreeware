...
LDFLAGS="-L%{_prefix}/lib64 -L%{_prefix}/lib -L/usr/lib64 -L/usr/lib $OPENSSL_LDFLAGS" \
CFLAGS="-O2" \
LIBS=-lpthread \
./configure \
    --prefix=%{_prefix} \
    --libdir=%{_prefix}/lib64 \
    --libexecdir=%{_prefix}/lib64 \
...
do
   ...
   if [ ! -e %{_prefix}/lib64/${lib} ]; then
       ln -s %{_builddir}/openldap-%{version}/64bit/libraries/${dirlib}/.libs/${lib} %{_prefix}/lib64/${lib}
   fi
done
...
# move slapd out of %{_prefix}/lib64
...
rm -f %{_prefix}/lib64/liblber.a
rm -f %{_prefix}/lib64/liblber.la
rm -f %{_prefix}/lib64/libldap.la
...
do
   dirlib="`echo  ${lib} | cut -d"." -f1`"
   if [ ! -e %{_libdir}/${lib} ]; then
       ln -s %{_builddir}/openldap-%{version}/32bit/libraries/${dirlib}/.libs/${lib} %{_libdir}/${lib}
   fi
done
...
rm -f %{_libdir}/liblber.a
rm -f %{_libdir}/liblber.la
rm -f %{_libdir}/libldap.la
...
for i in lber-2.4 ldap-2.4 ldap_r-2.4 ; do
    /usr/bin/ar -X64 -q ${RPM_BUILD_ROOT}%{_libdir}/lib${i}.a ${RPM_BUILD_ROOT}%{_prefix}/lib64/lib${i}.so*
done
...
%attr(0755,root,system) %{_prefix}/lib64/liblber-2.4.so*
%attr(0755,root,system) %{_prefix}/lib64/libldap-2.4.so*
%attr(0755,root,system) %{_prefix}/lib64/libldap_r-2.4.so*
