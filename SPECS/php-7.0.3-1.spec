...
preunAlternateLocation() {
    ...
    then
        [ -f  $1/conf/php.ini ] && rm -f $1/conf/php.ini
...
(/usr/IBMAHS/bin/httpd -version 2>&1 ; %{_prefix}/bin/httpd -version 2>&1 ;  %{_prefix}/apache2/httpd -version 2>&1) | grep -q "Apache/2.4"
if [ $? != 0 ]
then
...
postAlternateLocation() {
    ...
    then
        cp -p %{_sysconfdir}/php.ini $1/conf
...
if [ -f %{_prefix}/bin/httpd -a `%{_prefix}/bin/httpd -version 2>&1 | grep "Apache/2.4" | wc -l` = "1" ]
then
    addToHTTPDconf %{_sysconfdir}/httpd/conf/httpd.conf
fi
