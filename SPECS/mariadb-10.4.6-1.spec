...
#  community : (xlarge)
# BEWARE of redefinition via plugins defines.
%define feature                 xlarge
 # YES, AUTO, STATIC, DYNAMIC ou NO
# These static plugins work
%define archive                 STATIC
%define aria                    STATIC
%define blackhole               STATIC
%define federatedx              STATIC
%define feedback                STATIC
%define file_key_management     STATIC
%define ftexample               STATIC
%define innodb                  STATIC
%define locales                 STATIC
%define metadata_lock_info      STATIC
%define myisam                  STATIC
%define partition               STATIC
%define perfschema              STATIC
%define query_cache_info        STATIC
%define query_response_time     STATIC
# This static plugin does not work
%define connect                 NO
# These dynamic plugins work
%define audit_null              DYNAMIC
%define auth_ed25519            DYNAMIC
%define disks                   DYNAMIC
...
# These dynamic plugins do not work
%define spider                  NO
...
# If they do not work, we just unactivate them.
%define test_sql_discovery      STATIC
%define example                 NO
...
# TODO: Explain how these values are found
%define so_mariadb    3
...
Patch31:        mariadb-10.3.16-skip_unskip_test.patch
...
Patch42:        mariadb-10.4.6-plugin_dyn_export.patch
# Test perror
Patch43:        mariadb-10.4.6-errorcode.patch
...
# asynchronous operations stuff: Linux only!
...
%define all_folder ./BUILD ./BUILD-CMAKE ./CMakeLists.txt ./CONTRIBUTING.md ./COPYING ./CREDITS ./Docs ./EXCEPTIONS-CLIENT ./INSTALL-SOURCE ./INSTALL-WIN-SOURCE ./KNOWN_BUGS.txt ./README.md ./THIRDPARTY ./VERSION ./appveyor.yml ./client ./cmake ./config.h.cmake ./configure.cmake ./dbug ./debian ./extra ./include ./libmariadb ./libmysqld ./libservices ./man ./mysql-test ./mysys ./mysys_ssl ./pcre ./plugin ./randgen ./scripts ./sql ./sql-bench ./sql-common ./storage ./strings ./support-files ./tests ./unittest ./vio ./win ./wsrep-lib ./zlib
...
CFLAGS="-D_GNU_SOURCE -D_FILE_OFFSET_BITS=64 -D_LARGE_FILES -maix64"
# use mcmodel
...
gmake %{?_smp_mflags} VERBOSE=1 %{compile_flag}
...
(
  ...
  cd mysql-test
  perl ./mysql-test-run.pl --parallel=1 --force --retry=0 --ssl \
    --suite-timeout=900 --testcase-timeout=30 \
    --mysqld=--binlog-format=mixed --force-restart \
    --shutdown-timeout=60 --max-test-fail=5 \
    --skip-test=spider \
    --max-test-fail=9999 \
...
%if "%{spider}" != "NO"
 ...
 perl ./mysql-test-run.pl --parallel=1 --force --retry=0 \
...
mv liblibmysqld.so                libmysqld.so.%{so_mariadbd}
ln -s libmysqld.so.%{so_mariadbd} libmariadbd.so.%{so_mariadbd}
...
# Copy .so to lib
cp libmariadbd.so.%{so_mariadbd} ${RPM_BUILD_ROOT}%{_libdir64}
cp libmysqld.so.%{so_mariadbd}   ${RPM_BUILD_ROOT}%{_libdir64}
