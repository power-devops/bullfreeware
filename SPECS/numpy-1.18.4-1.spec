...
Source0:        https://github.com/%{name}/%{name}/releases/download/v%{version}/%{name}-%{version}.zip
Source1:        https://docs.scipy.org/doc/numpy/numpy-html-1.17.0.zip
Source100:      %{name}-%{version}-%{release}.build.log 
Patch1:         numpy-1.18.4-IEEE_except.patch
Patch2:         numpy-1.18.4-f2py.patch
Patch3:         numpy-1.18.4-RPATH.patch
...
%package -n python3.%{python3_minor_version}-numpy
Summary:        A fast multidimensional array facility for Python
License:        BSD
...
Provides:      python3-numpy = %{version}-%{release}
Provides:       libnpymath-static = %{version}-%{release}


%description -n python3.%{python3_minor_version}-numpy
NumPy is a general-purpose array-processing package designed to
efficiently manipulate large multi-dimensional arrays of arbitrary
records without sacrificing too much speed for small multi-dimensional
arrays.  NumPy is built on the Numeric code base and adds features
...
%package -n python3.%{python3_minor_version}-numpy-f2py
Summary:        f2py for numpy
Requires:       python3.%{python3_minor_version}-numpy = %{version}-%{release}
Requires:       python3-devel
Provides:       python3-f2py = %{version}-%{release}

%description -n python3.%{python3_minor_version}-numpy-f2py
This package includes a version of f2py that works properly with NumPy.


%package -n python3.%{python3_minor_version}-numpy-doc
Summary:	Documentation for numpy
Requires:	python3.%{python3_minor_version}-numpy = %{version}-%{release}
BuildArch:	noarch

%description -n python3.%{python3_minor_version}-numpy-doc
...
include_dirs = %{_includedir}
runtime_library_dirs = %{_libdir}

[openblas]
libraries = openblas
library_dirs = %{_libdir}
include_dirs = %{_includedir}/OpenBLAS_POWER6/
runtime_library_dirs = %{_libdir}
extra_link_args = -lopenblas
extra_compile_args = -maix32
...
(
  ...
  for fic in $(ls -1)
  do
    /opt/freeware/bin/sed -i 's|/opt/freeware/src/packages/BUILD/numpy-%{version}/64bit/python_venv/bin/python3_64|/usr/bin/env python3_64|' %{buildroot}%{_bindir}/$fic
    /opt/freeware/bin/sed -i 's|/opt/freeware/src/packages/BUILD/numpy-%{version}/32bit/python_venv/bin/python3_32|/usr/bin/env python3_32|' %{buildroot}%{_bindir}/$fic
  done
)
...
ln -s %{python3_sitearch64}/%{name}/core/include/numpy/ %{buildroot}%{_includedir}/numpy
...
ulimit -m unlimited
# ulimit -f unlimited
( CC="gcc -maix64" python runtests.py -m 'full' -v || true )
deactivate
...
ulimit -m unlimited
# ulimit -f unlimited
( CC="gcc -maix32" python runtests.py -m 'full' -v || true )
deactivate
...
%files -n python3.%{python3_minor_version}-numpy
%defattr(-,root,system,-)
%doc 32bit/LICENSE.txt
%doc 32bit/THANKS.txt 32bit/site.cfg.example
%{python3_sitearch64}/%{name}/__pycache__
%dir %{python3_sitearch64}/%{name}
%{python3_sitearch64}/%{name}/*.py*
%{python3_sitearch64}/%{name}/core
%{python3_sitearch64}/%{name}/distutils
%{python3_sitearch64}/%{name}/doc
%{python3_sitearch64}/%{name}/fft
%{python3_sitearch64}/%{name}/lib
%{python3_sitearch64}/%{name}/linalg
%{python3_sitearch64}/%{name}/ma
%{python3_sitearch64}/%{name}/random
%{python3_sitearch64}/%{name}/testing
%{python3_sitearch64}/%{name}/tests
%{python3_sitearch64}/%{name}/compat
%{python3_sitearch64}/%{name}/matrixlib
%{python3_sitearch64}/%{name}/polynomial
%{python3_sitearch64}/%{name}-*.egg-info
%exclude %{python3_sitearch64}/%{name}/LICENSE.txt

%{python3_sitearch32}/%{name}/__pycache__
%dir %{python3_sitearch32}/%{name}
%{python3_sitearch32}/%{name}/*.py*
%{python3_sitearch32}/%{name}/core
%{python3_sitearch32}/%{name}/distutils
%{python3_sitearch32}/%{name}/doc
%{python3_sitearch32}/%{name}/fft
%{python3_sitearch32}/%{name}/lib
%{python3_sitearch32}/%{name}/linalg
%{python3_sitearch32}/%{name}/ma
%{python3_sitearch32}/%{name}/random
%{python3_sitearch32}/%{name}/testing
%{python3_sitearch32}/%{name}/tests
%{python3_sitearch32}/%{name}/compat
%{python3_sitearch32}/%{name}/matrixlib
%{python3_sitearch32}/%{name}/polynomial
%{python3_sitearch32}/%{name}-*.egg-info
%exclude %{python3_sitearch32}/%{name}/LICENSE.txt

%{_includedir}/numpy

%files -n python3.%{python3_minor_version}-numpy-f2py
%defattr(-,root,system,-)
%{_bindir}/f2py*
%{python3_sitearch32}/%{name}/f2py
%{python3_sitearch64}/%{name}/f2py

%files -n python3.%{python3_minor_version}-numpy-doc
%defattr(-,root,system,-)
%doc docs/*
