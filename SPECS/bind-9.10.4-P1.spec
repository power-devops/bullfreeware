...
application with "cc or xlc : -q64" or "gcc or gxlc -maix64" .
...
sed -e 's;|\*-ibm-aix5\.\*;|*-ibm-aix5.*|*-ibm-aix6.*;'      < bin/tests/system/ifconfig.sh  > bin/tests/system/ifconfig.sh.tmp.$$
mv  bin/tests/system/ifconfig.sh.tmp.$$ bin/tests/system/ifconfig.sh

# Duplicate source for 32 & 64 bits
/usr/bin/rm -rf /tmp/%{name}-%{VERSION}-32bit
...
mkdir 64bit
cp -pr 32bit/* 64bit/
cp .gitattributes 32bit/
mv .gitattributes 64bit/
...
PACK_ROOT=$PWD
cd 64bit
export CC="/usr/vac/bin/gxlc -maix64"
...
# second : build the 32-bit version
cd $PACK_ROOT
cd 32bit
export CC="/usr/vac/bin/gxlc -maix32"
...
mkdir tmp
find ${RPM_BUILD_ROOT} -type f -name '*.a' |  grep  -e /lib64/ |
while read f; do
  fbn=$(basename $f);
  ls -l $f;
  cp $f tmp/$fbn;
...
cd tmp
find ${RPM_BUILD_ROOT}  -type f -name '*.a' | grep -v -e /lib64/ |
while read f; do
  bn=$(basename $f);
  /usr/bin/ar -X64 xv $bn;
  /usr/bin/ar -q -X64 $f *.o;
  rm -f *.o;
  rm -f $bn;
done
cd ..
...
/usr/bin/ar -q -X64 ${RPM_BUILD_ROOT}%{_libdir}/%{name}.a 
...
/usr/bin/rm -f ${RPM_BUILD_ROOT}%{_sysconfdir}/bind.keys

# Install isc/errno2result.h header
mkdir -p ${RPM_BUILD_ROOT}%{_includedir}/bind9/isc

/usr/bin/install -M 644 -f ${RPM_BUILD_ROOT}%{_includedir}/bind9/isc lib/isc/unix/errno2result.h
...
/usr/bin/install -M 644 -f ${RPM_BUILD_ROOT}%{_sysconfdir}/ bind.keys
mv ${RPM_BUILD_ROOT}%{_sysconfdir}/bind.keys ${RPM_BUILD_ROOT}%{_sysconfdir}/named.iscdlv.key
/usr/bin/install -M 644 -f ${RPM_BUILD_ROOT}%{_sysconfdir}/ %{SOURCE36}


# sample bind configuration files for %%doc:
mkdir -p sample/etc sample/var/named/data sample/etc/slaves
/usr/bin/install -M 644 -f sample/etc/ %{SOURCE25}
...
#/usr/bin/install -M 644 -f ${RPM_BUILD_ROOT}/etc/ named.conf.default
#mv ${RPM_BUILD_ROOT}/etc/named.conf.default ${RPM_BUILD_ROOT}/etc/named.conf
#/usr/bin/install -M 644 -f ${RPM_BUILD_ROOT}/etc/ sample/etc/named.rfc1912.zones
#/usr/bin/install -M 644 -f ${RPM_BUILD_ROOT}/var/named/{named.ca,named.localhost,named.loopback,named.empty}  sample/var/named
...
/usr/bin/install -M 644 -f ${RPM_BUILD_ROOT}%{_sysconfdir}/tmpfiles.d/ %{SOURCE35}
mv ${RPM_BUILD_ROOT}%{_sysconfdir}/tmpfiles.d/bind.tmpfiles.d ${RPM_BUILD_ROOT}%{_sysconfdir}/tmpfiles.d/named.conf

mkdir -p ${RPM_BUILD_ROOT}%{_sysconfdir}/rwtab.d
/usr/bin/install -M 644 -f ${RPM_BUILD_ROOT}%{_sysconfdir}/rwtab.d/ %{SOURCE43}
...
%doc 32bit/doc/arm/*html 32bit/doc/arm/*pdf
%doc 32bit/sample/
...
%changelog
* Fri Jul 1 2016 CEST 2016 J. Girardet 
- update to 9.10.4-P1 AIX
