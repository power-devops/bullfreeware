...
BuildRequires: python3-devel >= 3.8, gettext-devel, perl(perl) >= 5.30
...
rm -rf ..?* .[!.]* *
mv       /tmp/%{name}-%{version}-vim vim-32
...
%if %{with gui}
cp -pr vim-32 gvim-32
cp -pr vim-32 gvim-64
%endif
...
%if %{with gui}
cd ../../gvim-64/src
build_vim "huge" "--enable-python3interp --with-x --enable-xim --enable-gui=gtk2 --enable-gtk2-check"
cp vim gvim_64
%endif

cd ../../vim-64/src
build_vim "huge" "--enable-python3interp --without-x --enable-gui=no --enable-fontset"
cp vim vim_64

# Common to all 32 bits
export OBJECT_MODE=32
export CC="/opt/freeware/bin/gcc -maix32 -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE -D_LARGE_FILES"
...
cp vim-32/src/vim_32   ${RPM_BUILD_ROOT}%{_bindir}
cp vim-32/src/vimtutor ${RPM_BUILD_ROOT}%{_bindir}/vimtutor_32
cp vi-64/src/vi_64     ${RPM_BUILD_ROOT}%{_bindir}
cp vi-32/src/vi_32     ${RPM_BUILD_ROOT}%{_bindir}
%if %{with gui}
cp gvim-64/src/gvim_64 ${RPM_BUILD_ROOT}%{_bindir}
cp gvim-32/src/gvim_32 ${RPM_BUILD_ROOT}%{_bindir}
%endif
