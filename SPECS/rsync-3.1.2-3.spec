...
patch -p1 -i patches/write-devices.diff
patch -p1 -i patches/tru64.diff
...
#   ./prepare-source
    ./configure \
        ...
        --with-included-popt \
        --enable-acl-support
#        --disable-largefiles  no more supported v3.1.2
...
( cd $RPM_BUILD_ROOT/%{_prefix}
  ...
  do
      ...
      do
          ...
          mv $fic $fic"_64"
          ln -s $fic"_32" $fic  # By default, binaries link to 64bit but 64bit pb
      done
      cd -
  done
)
...
%attr(0755,root,system) %{_bindir}/rsync_32
#%attr(0755,root,system) %{_bindir}/rsync_32
/usr/bin/rsync

%files ssl-client
%{_bindir}/rsync-ssl
%{_bindir}/stunnel-rsync
...
%changelog
##########
* Fri Apr 28 2017 Daniele Silvestre <daniele.silvestre@atos.net> - 3.1.2-3
- Fix link problem or rsync binary
- Add ssl-client and ssl-daemon package like defined in sambaorg rsync.spec

* Wed Nov 15 2016 Daniele Silvestre <daniele.silvestre@atos.net> - 3.1.2-2
- Default delivery : 32 bit compiled with GCC compiler
  rsync pb with rsync for char/block devices files in 64 bit mode

* Wed Oct 19 2016 Daniele Silvestre <daniele.silvestre@atos.net> - 3.1.2-1
- Update to version 3.1.2
- Build 32 and 64 bit
- Compile with GCC or XLC
- Default delivery : 64 bit, compiled with GCC compiler
- Add tests
