...
Source12: libcrypto.so.1.0.0-aix32
Source13: libcrypto.so.1.0.0-aix64
Source14: libssl.so.1.0.0-aix32
Source15: libssl.so.1.0.0-aix64
...
${CC} -qmkshrobj *.o -o ../libcrypto.so.1.0.1 -bexpall -bernotok
cd ..
rm -rf libcrypto.a .tmplibdir
/usr/bin/ar -rv libcrypto.a libcrypto.so.1.0.1

mkdir .tmplibdir
cd .tmplibdir
/usr/bin/ar -x ../libssl.a
${CC} -qmkshrobj *.o -o ../libssl.so.1.0.1 -bexpall -bernotok -L.. -lcrypto -blibpath:/opt/freeware/lib:/usr/lib:/lib 
cd ..
rm -rf libssl.a .tmplibdir
/usr/bin/ar -rv libssl.a libssl.so.1.0.1

gmake

mkdir -p 64
mv libcrypto.so.1.0.1 libssl.so.1.0.1 64/
gmake clean
...
/usr/bin/ar -X64 -q libcrypto.a 64/libcrypto.so.1.0.1
/usr/bin/ar -X64 -q libssl.a 64/libssl.so.1.0.1

# Add the older 0.9.7l, 0.9.8x and 1.0.0j shared members for compatibility with older apps
...
cp %{SOURCE12} libcrypto.so.1.0.0
...
ln -s libcrypto.so.1.0.1 libcrypto.so
ln -s libssl.so.1.0.1 libssl.so
...
# change these to be full-path links
  ...
  ln -sf %{_libdir}/libcrypto.so.1.0.1 .
  ln -sf %{_libdir}/libssl.a .
  ln -sf %{_libdir}/libssl.so .
  ln -sf %{_libdir}/libssl.so.1.0.1 .

  cd ../lib64
  rm ./libcrypto.* ./libssl.*
  ln -sf %{_libdir64}/libcrypto.so .
  ln -sf %{_libdir64}/libcrypto.so.1.0.1 .
  ln -sf %{_libdir64}/libssl.so .
  ln -sf %{_libdir64}/libssl.so.1.0.1 .
