...
# Versions for version 0.21 :
%define	gettext_libintl_version		8
%define	gettext_libasprintf_version	0
%define	gettext_libgettextpo_version	0
%define	gettext_libtextstyle_version	0
...
## Internal use only. Commented; can be removed in future version.
...
## Current version is also 0; so, useless.
...
Patch0:		%{name}-0.20.1-aixconf.patch
Patch1:		%{name}-0.21-aixAddCast.patch
...
Patch4:     %{name}-0.20.1-libtool-remove-duplicates-opt-freeware-in-libpath.patch
...
Patch5:     %{name}-0.21-fix-iconv-and-intl-order-for-libgettexlib.patch
...
Patch6:     %{name}-0.20.1-configure-force-GNU-getline-and-getdelim.patch
...
## No more add old .so for these lib. Can be removed in future version.
...
#cVilliers-Saint-Frédéricp %{SOURCE5}                                                      libgettextlib-0.18.3.so
...
%changelog
* Mon Feb 08 2021 Bullfreeware Continuous Integration <bullfreeware@atos.net> - 0.21-1
- Update to 0.21
- Stop adding some old .so in .a: it is internal lib, or the same version
- No more compatible AIX 6.1

* Fri Sep 18 2020 Clément Chigot <clement.chigot@atos.net - 0.20.1-3
- Correctly run AIXtoolbox old gettext handler in %pre and not in %preun
- Remove --disable-rpath from configure
- Remove /usr/lib/libintl.a
