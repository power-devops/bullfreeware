...
Source1: ftp://ftp.gnu.org/pub/gnu/leim-21.4.tar.gz
Patch0: emacs-%{version}-info.info.patch
...
# See cp info/info.info below
%patch0 -p1


%build

cp info/info.info info/emacs.info.info
...
export PATH="/usr/bin:/opt/freeware/bin:$PATH"
...
# This is a work-around for an issue with libtoolize (seen on AIX 6.1 on 2016/02)
echo "libtoolize version 2.4.3 removes emacs-%{version}/build-aux/snippet/*"
echo "libtoolize version 2.4.6 removes emacs-%{version}/build-aux/depcomp and more"
# ls -l /opt/freeware/src/packages/BUILD/emacs-%{version}/build-aux/snippet/c++defs.h
# ls -l /opt/freeware/src/packages/BUILD/emacs-%{version}/build-aux/depcomp
#libtoolize --force --copy --verbose --debug
libtoolize --force --copy
# ls -l /opt/freeware/src/packages/BUILD/emacs-%{version}/build-aux/snippet/c++defs.h
# ls -l /opt/freeware/src/packages/BUILD/emacs-%{version}/build-aux/depcomp
cd ..
/opt/freeware/bin/tar zxf /opt/freeware/src/packages/SOURCES/emacs-%{version}.tar.gz emacs-%{version}/build-aux/snippet/*
/opt/freeware/bin/tar zxf /opt/freeware/src/packages/SOURCES/emacs-%{version}.tar.gz emacs-%{version}/build-aux/depcomp
ls -l /opt/freeware/src/packages/BUILD/emacs-%{version}/build-aux/snippet/c++defs.h
ls -l /opt/freeware/src/packages/BUILD/emacs-%{version}/build-aux/depcomp
...
CFLAGS="$RPM_OPT_FLAGS -I/opt/freeware/include -I/usr/include" \
  ../configure ${CONFOPTS} --with-x-toolkit=lucid --with-gif=no

gmake

cd ..

# xlc (v12 & v13) refuses this:
#	../../lwlib/../src/lisp.h", line 1254.18: 1506-046 (S) Syntax error
#	enum { ALIGNOF_STRUCT_LISP_VECTOR = alignof (union { struct vectorlike_header a; Lisp_Object b; }) };
# though gcc accepts it...

# configure: checking for gio-2.0 >= 2.26... yes CFLAGS='-qthreaded ....
# gcc: error: unrecognized command line option '-qthreaded'
# Compilation continues when -qthreaded removed (by hand...)
# GSETTINGS_CFLAGS='-qthreaded -D_THREAD_SAFE
# GSETTINGS_CFLAGS=`"$PKG_CONFIG" --cflags "gio-2.0 >= 2.26" 2>&5
# configure : line 12146
#  /opt/freeware/bin/pkg-config --cflags 'gio-2.0 >= 2.26'  ====>
#	-qthreaded -D_THREAD_SAFE -I/opt/freeware/include/glib-2.0 -I/opt/freeware/lib/glib-2.0/include 
# grep qthread /opt/freeware/lib/pkgconfig/*.pc
# -qthreaded comes from: /opt/freeware/lib/pkgconfig/gmodule-no-export-2.0.pc 
#   Unknown keyword 'Libs.private' in '/opt/freeware/lib/pkgconfig/gmodule-no-export-2.0.pc'
#	  line>Cflags: -qthreaded -D_THREAD_SAFE
...
echo "RPM_BUILD_ROOT%{_prefix}: $RPM_BUILD_ROOT%{_prefix}"
...
# NO !! generates .gz.gz files !!!    gzip -9nf $RPM_BUILD_ROOT%{_prefix}/info/*
...
mv $RPM_BUILD_ROOT%{_prefix}/man/man1/ctags.1.gz $RPM_BUILD_ROOT%{_prefix}/man/man1/gctags.1.gz
...
# 24.5 : leim moved to lisp/leim so that leim-filelist is included in core-filelist
#find $RPM_BUILD_ROOT%{_prefix}/share/emacs/%{PACKAGE_VERSION}/lisp/leim \
...
%post X11
if [ ! -x %{_prefix}/bin/emacs -a ! -L %{_prefix}/bin/emacs ]; then
  ln -sf emacs-X11 %{_prefix}/bin/emacs
fi
...
#%dir %{_prefix}/share/emacs/%{PACKAGE_VERSION}/lisp/leim
...
#%defattr(-,root,system)
#%{_prefix}/share/emacs/%{PACKAGE_VERSION}/lisp/leim/leim-list.el
...
%changelog
* Thu Mar 24 2017 Michael Wilson <michael.a.wilson@atos.net> 25.1-1
- Update to version 25.1

* Mon Mar 23 2017 Tony Reix <tony.reix@atos.net> 24.5-3
- Rebuild with gettext 0.19.8-1 in order to no more need __dbargs & Co

* Thu Jun 23 2016 Tony Reix <tony.reix@atos.net> 24.5-2
- Remove emacs with X11 version

* Thu Mar 2 2016 Tony Reix <tony.reix@atos.net> 24.5-1
- Update to version 24.5

* Thu Feb 25 2016 Tony Reix <tony.reix@atos.net> 23.4-1
