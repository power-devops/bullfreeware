...
# GCC may be used to build a future (4.4.5) version
#
# samba-4.3.8 contains new security fixes and a number of code changes
# Using the recent SPEC file for samba-4.3.6-2 for this new version
#
...
# Patch0: samba-4.3.6-security-2016-04-12.patch
...
# Patch3: samba-4.2.0-Texpect.patch == functions for err() & errx() included
Patch4: samba-4.2.0-TdbAIX.patch
# Patch5: samba-4.2.0-netlogon_creds_cli.patch == correction in 4.3.4
...
Patch9: samba-4.3.4-resolv_wrapper.patch
#Patch10: samba-4.3.6-unix_msg.patch
...
# May be Requires: krb5-libs >= 1.10 (not yet on Bullfreeware)
...
cp samba-4.3.8/source3/script/mksmbpasswd.sh ${RPM_BUILD_ROOT}%{_bindir}/
chmod 0755 $RPM_BUILD_ROOT%{_bindir}/mksmbpasswd.sh

cp samba-4.3.8/packaging/RHEL/setup/smbusers ${RPM_BUILD_ROOT}/etc//%{name}/smbusers
chmod 0644 ${RPM_BUILD_ROOT}/etc//%{name}/smbusers

cp samba-4.3.8/packaging/printing/smbprint ${RPM_BUILD_ROOT}%{_bindir}/
...
cp samba-4.3.8/examples/LDAP/%{name}.schema ${RPM_BUILD_ROOT}/etc/openldap/schema/%{name}.schema
chmod 0644 ${RPM_BUILD_ROOT}/etc/openldap/schema/%{name}.schema

# winbind krb5 locator
mkdir -p ${RPM_BUILD_ROOT}%{_libdir}/krb5/plugins/libkrb5
cp samba-4.3.8/bin/shared/winbind_krb5_locator.so ${RPM_BUILD_ROOT}%{_libdir}/krb5/plugins/libkrb5/winbind_krb5_locator.so
...
#done
cp samba-4.3.8/bin/default/source3/libsmb/smbclient.pc ${RPM_BUILD_ROOT}%{_libdir}/pkgconfig/ 
cp samba-4.3.8/bin/default/source3/libnet/netapi.pc ${RPM_BUILD_ROOT}%{_libdir}/pkgconfig/ 
cp samba-4.3.8/bin/default/nsswitch/libwbclient/wbclient.pc ${RPM_BUILD_ROOT}%{_libdir}/pkgconfig/

mkdir -p ${RPM_BUILD_ROOT}/etc/sysconfig
cp %{SOURCE4} ${RPM_BUILD_ROOT}/etc/sysconfig/%{name}
chmod 0644 ${RPM_BUILD_ROOT}/etc/sysconfig/%{name}

# cp -r samba-4.3.8/source3/lib/netapi/examples/netdomjoin-gui ${RPM_BUILD_ROOT}%{_sbindir}/
# chmod 0755 ${RPM_BUILD_ROOT}%{_sbindir}/netdomjoin-gui

# mkdir -p ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}
# cp samba-4.3.8/source3/lib/netapi/examples/netdomjoin-gui/%{name}.ico ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/%{name}.ico
# chmod 0644 ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/%{name}.ico

# cp samba-4.3.8/source3/lib/netapi/examples/netdomjoin-gui/logo.png ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/logo.png
# chmod 0644 ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/logo.png

# cp samba-4.3.8/source3/lib/netapi/examples/netdomjoin-gui/logo-small.png ${RPM_BUILD_ROOT}%{_datadir}/pixmaps/%{name}/logo-small.png
...
# samba_common
ln -sf  libtalloc.so.2.1.3     ${RPM_BUILD_ROOT}%{_libdir}/samba/libtalloc.so
ln -sf  libpytalloc-util.so.2.1.3     ${RPM_BUILD_ROOT}%{_libdir}/samba/libpytalloc-util.so
ln -sf  libtevent.so.0.9.25     ${RPM_BUILD_ROOT}%{_libdir}/samba/libtevent.so
ln -sf  libtdb.so.1.3.7     ${RPM_BUILD_ROOT}%{_libdir}/samba/libtdb.so
ln -sf  libldb.so.1.1.21     ${RPM_BUILD_ROOT}%{_libdir}/samba/libldb.so
...
# Internal or independent TALLOC library/package (2.1.3)
