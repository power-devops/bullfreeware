...
Source0: https://www.apache.org/dist/%{name}/%{name}-%{version}.tar.bz2
...
BuildRequires: openldap >= 2.4.35
%if %{included_apr} == 0
BuildRequires: apr-devel >= 1.5.2
BuildRequires: apr-util-devel >= 1.5.4-2
%endif
...
%if %{included_apr} == 1
CMD="true"
HTTPD_INSTALLED=`/usr/bin/rpm -q -i httpd 2>&1 || true`
if [ "${HTTPD_INSTALLED}" != "package httpd is not installed" ]
then
    echo "ERROR: httpd must be uninstalled before building a newer version of httpd"
    CMD="false"
fi
APR_INSTALLED=`/usr/bin/rpm -q -i apr 2>&1 || true`
if [ "${APR_INSTALLED}" != "package apr is not installed" ]
then
    echo "ERROR: apr must be uninstalled before building a newer version of httpd"
    CMD="false"
fi
APR_UTIL_INSTALLED=`/usr/bin/rpm -q -i apr-util 2>&1 || true`
if [ "${APR_UTIL_INSTALLED}" != "package apr-util is not installed" ]
then
    echo "ERROR: apr-util must be uninstalled before building a newer version of httpd"
    CMD="false"
fi
${CMD}
%endif
...
ln -s %{localstatedir}/lib/httpd ${RPM_BUILD_ROOT}%{_sysconfdir}/httpd/state
...
%changelog
* Fri Jul 20 2018 Michael Wilson <michael.a.wilson@atos.net> 2.4.34-1
- Update to version 2.4.34

* Tue Feb 07 2017 Tony Reix <tony.reix@atos.net> 2.4.25-1
- Update to version 2.4.25

* Mon Jul 04 2016 Matthieu Sarter <matthieu.sarter.external@atos.net> 2.4.20-1
- Update to version 2.4.20

* Fri Mar 30 2016 Matthieu Sarter <matthieu.sarter.external@atos.net> 2.4.18-1
