...
Patch0: %{name}-%{version}-2018fallfixes.patch
...
(
   ### AIX stuff
   # Create the 32bit libxxx.a shared libraries
   # Add libxxx.so object to libxxx.a
   /usr/bin/strip -X32 -e ${RPM_BUILD_ROOT}%{_libdir}/libzip.so
   $AR -X32 -q ${RPM_BUILD_ROOT}%{_libdir}/libzip.a ${RPM_BUILD_ROOT}/%{_libdir}/libzip.so
   mv ${RPM_BUILD_ROOT}/%{_libdir}/libzip.so ${RPM_BUILD_ROOT}/%{_libdir}/libzip.so32bit
#  ### many packages have to manage libxxx.so.version objects
#  ### but libzip don't
#  ##$AR -X32 -q ${RPM_BUILD_ROOT}%{_libdir}/lib"$lib".a   ${RPM_BUILD_ROOT}/%{_libdir}/lib"$lib".so"$SUFIX"
#

    # Identify binaries as 32 bits binaries
    # strip binaries
    cd  ${RPM_BUILD_ROOT}
    for dir in bin
    do
...
# ==========================
cd 64bit
$MAKE --trace --print-directory DESTDIR=${RPM_BUILD_ROOT} install 

# Add the 64-bit shared objects to the shared library containing already the 32-bit shared objects
$AR -X64 -q ${RPM_BUILD_ROOT}%{_libdir}/libzip.a ${RPM_BUILD_ROOT}%{_libdir}/libzip.so 

### many packages have to manage libxxx.so.version objects
### but libzip don't
##$AR -X32_64 -q ${RPM_BUILD_ROOT}%{_libdir}/lib"$lib".a ${RPM_BUILD_ROOT}/%{_libdir}/lib"$lib".so"$SUFFIX"

# Create the symlink to lib64
mkdir -p ${RPM_BUILD_ROOT}%{_libdir64}
(
   cd ${RPM_BUILD_ROOT}%{_libdir64}
   ln -s ../lib/libzip.a .
)
mv ${RPM_BUILD_ROOT}/%{_libdir}/libzip.so ${RPM_BUILD_ROOT}/%{_libdir64}/libzip.so
mv ${RPM_BUILD_ROOT}/%{_libdir}/libzip.so32bit ${RPM_BUILD_ROOT}/%{_libdir}/libzip.so

cp ${RPM_BUILD_ROOT}%{_libdir}/libzip.so ${RPM_BUILD_ROOT}%{_libdir}/libzip.so.5 
cp ${RPM_BUILD_ROOT}%{_libdir64}/libzip.so ${RPM_BUILD_ROOT}%{_libdir64}/libzip.so.5 

(
    # Identify binaries as 64 bits binaries
    # Strip binaries
    # Choose 32 or 64 as default binaries version
    cd  ${RPM_BUILD_ROOT}
    ...
    do
        ...
        do
            ...
            mv ${f} ${f}_64
            # By default, binaries link to 64bit
