...
Patch1: db-4.4.20-1.patch
Patch2: db-4.4.20-2.patch
Patch3: db-4.4.20-3.patch
...
%prep
%setup -q
%patch1 -b .db-4.4.20-1
%patch2 -b .db-4.4.20-2
%patch3 -p1 -b .db-4.4.20-3
...
LIBSO_LIBS=-lpthread ../dist/configure --prefix=%{_prefix} \
	--enable-compat185 --enable-shared --enable-static \
...
cd ..
/usr/bin/ar -crv build_unix/libdb-4.4.a \
        build_unix/.libs/libdb-4.4.so
...
#  into same archive
cd ../../..
/usr/bin/ar -q build_unix/libdb-4.4.a \
    64bit/%{name}-%{version}/build_unix/.libs/libdb-4.4.so
%endif #BUILD64
