...
Source0: ftp://ftp.apache.org/apache/dist/httpd-%{apache_vers}.tar.bz2
URL: http://www.apache.org
License: Apache
BuildRoot: %{_tmppath}/httpd-root
Provides: webserver
Prefix: %{_prefix}
...
OPTIM="$OPTIM" \
./configure \
    --with-included-apr --prefix=%{prefix}/apache \
...
# XXX - hack
[[ ! -d /opt/freeware/apache/lib ]] && mkdir -p /opt/freeware/apache/lib
[[ ! -f /opt/freeware/apache/lib/libapr-1.so ]] && \
  cp ./srclib/apr/.libs/libapr-1.so %{prefix}/apache/lib/libapr-1.so

make LIBPATH="$RPM_BUILD_ROOT/opt/freeware/apache/lib:/opt/freeware/lib" DESTDIR=$RPM_BUILD_ROOT root=$RPM_BUILD_ROOT install

/usr/bin/strip $RPM_BUILD_ROOT/%{prefix}/apache/bin/*  || :

# Link files in /usr/bin
(cd $RPM_BUILD_ROOT
 mkdir -p usr/bin
 cd usr/bin
 ln -sf ../..%{prefix}/apache/bin/* .
 cd -
)

# Link files in /usr/include
( cd /usr/include
for file in $(find /usr/local/apache2/include)
do
  ln -sf $file .
done
)
...
%config /%{prefix}/apache/conf/httpd.conf
%config /%{prefix}/apache/conf/magic
%config /%{prefix}/apache/conf/mime.types
%dir %{prefix}/apache/conf
%dir %{prefix}/apache/logs
%dir %{prefix}/apache/lib
%dir %{prefix}/apache
%dir %{prefix}/apache/cgi-bin
%{prefix}/apache/cgi-bin/*
%dir %{prefix}/apache/build
%{prefix}/apache/build/*
%dir %{prefix}/apache/modules
