...
# Required ONLY for version 2.3.6
Patch1: unixODBC-2.3.6-0001-sed.patch
...
for f in ${RPM_BUILD_ROOT}%{_libdir}/lib*.so ; do
    ...
    /usr/bin/ar -q -X32 ${RPM_BUILD_ROOT}%{_libdir}/${baselib}.a ${f}.%{APIrelease}
    strip -X32 -e ${f}
    /usr/bin/ar -q -X32 ${RPM_BUILD_ROOT}%{_libdir}/${baselib}.a ${f}
    lib64=`echo ${f} | sed 's|/opt/freeware/lib|/opt/freeware/lib64|g'`
    /usr/bin/ar -q -X64 ${RPM_BUILD_ROOT}%{_libdir}/${baselib}.a ${lib64}.%{APIrelease}
    strip -X64 -e ${lib64}
