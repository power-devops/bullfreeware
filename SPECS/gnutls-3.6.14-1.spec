...
Patch1: %{name}-3.6.7-netstat.patch
Patch2: %{name}-3.6.7-rpath.patch
# getline is somehow broken in AIX.  Using it along with fmemopen works in a weird way
Patch4: %{name}-3.6.7-aix-getline-broken.patch

# Following taken from Fedora 33
Patch5: %{name}-3.6.14-fix-iovec-memory-leak.patch

# Following voids abort/errpt due to test-ciphers-api
Patch6: %{name}-3.6.14-slow_tests.patch

%define _libdir64 %{_prefix}/lib64

BuildRequires: libgcrypt-devel >= 1.2.4, gettext-devel
BuildRequires: zlib-devel, libtasn1-devel >= 4.3
BuildRequires: readline-devel >= 5.2
BuildRequires: nettle-devel >= 3.5
BuildRequires: p11-kit-devel >= 0.21.3
BuildRequires: autogen-libopts >= 5.18
BuildRequires: autogen >= 5.18
...
%patch5 -p1 -b .iovecMemLeak

# Following voids abort/errpt due to test-ciphers-api
%patch6 -p1 -b .slowTests
...
export CFLAGS="-D_LINUX_SOURCE_COMPAT -DAF_LOCAL=AF_UNIX -O2"

# first build the 64-bit version
cd 64bit

#export CC="xlc -q64 -D_LARGE_FILES"
export CC="gcc -maix64 -D_LARGE_FILES -g"
...
export CC="gcc -maix32 -D_LARGE_FILES -g"
export AR="/usr/bin/ar -X32"
export OBJECT_MODE=32
export LDFLAGS="-L/opt/freeware/lib -Wl,-blibpath:/opt/freeware/lib:/usr/lib:/lib -Wl,-bmaxdata:0x80000000 -lcompat-getopt"
export CCLD="$CCLD -L../../lib/.libs"
echo "GCC: "
gcc --version
...
(
  ...
  for f in certtool gnutls-cli gnutls-cli-debug gnutls-serv psktool srptool p11tool
  do
    mv ${f} ${f}_64
  done
)
...
mkdir -p ${RPM_BUILD_ROOT}/etc/gnutls
cp %{SOURCE7} ${RPM_BUILD_ROOT}/etc/gnutls/
...
%changelog
* Mon Jun 15 2020 Michael Wilson <michael.a.wilson@atos.net> - 3.6.14-1
- Updated to version 3.6.14
- Patch taken from Fedora 33
- Add patch to W/A 2 test crashes - cipher-api/sigabrt and hash-large/PGSP_KILL
- Add BuildRequires diffutils for tests (diff -B)

* Mon Mar 30 2020 Étienne Guesnet <etienne.guesnet.external@atos.net> - 3.6.32-1
- Updated to 3.6.12
- Stop providing links in /usr

* Fri Oct 18 2019 Ayappan P <ayappap2@in.ibm.com> - 3.6.7-2
- Fix hardcoding of .pc file 
- Remove all .la files
- AIX getline broken, so enable GNU getline code 

* Tue May 14 2019 Ravi Hirekurabar <rhirekur@in.ibm.com> - 3.6.7-1
- Updated to 3.6.7

* Mon May 06 2019 Ravi Hirekurabar <rhirekur@in.ibm.com> - 3.6.4-2
- Rebuilt with nettle-3.4.1 to fix security vulnerability 
- Built with --with-system-priority-file=/etc/gnutls/default-priorities
- Avoided static linking to gcc 

* Thu Dec 20 2018 Ravi Hirekurabar <rhirekur@in.ibm.com> - 3.6.4-1 
- Updated to 3.6.4

* Wed May 17 2017 Ravi Hirekurabar<rhirekur@in.ibm.com> - 3.5.10-1
