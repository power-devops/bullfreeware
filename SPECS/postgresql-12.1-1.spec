...
Patch91:  0001-Add-huge-page-support-for-AIX.patch
Patch92:  0002-Add-huge-page-support-for-AIX-shared_memory_type_sysv.patch
...
%description static
The postgresql-static package provides the PostgreSQL static libraries.
...
#	returns:  perl(:MODULE_COMPAT_5.30.0) is needed by postgresql-plperl-12.1-1.ppc
#	However:  rpm -q --whatprovides "perl(:MODULE_COMPAT_5.30)"   --->   perl-5.30.0-1.ppc !!!
Requires: perl(:MODULE_COMPAT_%(eval "`/opt/freeware/bin/perl -V:version | cut -d. -f1,2 | sed \"s/$/\';/\"`"; echo $version))
Requires: perl >= 5.8.8
...
export GLOBAL_CC_OPTIONS="-O${OPTIMIZATION} $GLOBAL_CC_OPTIONS_TUNE"
...
if [ ${OPTIMIZATION} -eq 3 ]
then
	export XLC_strict="-qstrict"
fi

export GLOBAL_CC_OPTIONS="-O${OPTIMIZATION} ${XLC_strict} $GLOBAL_CC_OPTIONS_TUNE"
...
#   ulimit -f unlimited   no more works with AIX 7.2 TL3 and non-root user.
	...
	ulimit -f 1000000 export LDR_CNTRL=SHMPSIZE=64K ; $MAKE -k MAX_CONNECTIONS=5 check-world EXTRA_TESTS=$EXTRA_TESTS_LIST 2>&1; exit 0)"
...
# Put 64bit static *.o into 32bit static library and create symlink from lib64 to lib32.
(
  cd ${RPM_BUILD_ROOT}%{_libdir64}
  for i in common feutils port
  do
    mkdir $i
    (
      cd $i
      /usr/bin/ar -X64 -x        ../libpg$i.a
      /usr/bin/ar -X64 -q ../../lib/libpg$i.a *
    )
    rm -rf $i
    rm libpg$i.a
    ln -sf ../lib/libpg$i.a .
...
# contrib stuff is now placed in a pgsql directory instead of postgresql.
# Add a symlink
(
  for dir in %{_libdir64} %{_libdir} %{_datadir} %{_includedir}
  do
    (
...
%{_bindir}/pg_config*
%{_bindir}/pg_dump*
# Duplicate with pg_dump*	%{_bindir}/pg_dumpall*
%{_bindir}/pg_isready*
...
# Dynamic library
#	Now delivered only by postgresql-libs
#	%{_libdir}/libecpg.a
#	%{_libdir}/libecpg.so*
...
# link from lib64 to lib for dynamics library
#	Now delivered only by postgresql-libs
#	%{_libdir64}/libpq.a
#	%{_libdir64}/libecpg.a
...
%{_bindir}/pg_config*
%{_includedir}/*
# Create link from /usr to /opt/freeware
#	/usr/bin/ecpg*
#	/usr/bin/pg_config*
#	/usr/include/*
