...
#   libgo tests
# dotests = 2 for:
#   compilers tests + ppc libraries' tests
#   ppc libgo will also run ppc64, pthread and pthread/ppc64 tests unlike others. 
# dotests = 3 for: (about 20 hours...)
#   dotests = 2
#   tests for ppc64 pthread pthread/ppc64
#   libraries: libstdc++, libatomic, libgomp
%global do_install 1
...
Source0: ftp://ftp.gnu.org/gnu/%{name}/%{name}-%{version}/%{name}-%{gcc_version}.tar.xz
# Used for snapshots:
#Source0: ftp://ftp.gnu.org/gnu/%{name}/%{name}-%{version}/%{name}-%{gcc_version}-%{release}.tar.xz
...
Patch3:  gcc-9.1.0-gotools-fix-Makefile-v2.patch
...
# Fix libgo tests.
Patch104:   gcc-9.1.0-libgo-os-user-disable-TestGroupIds-for-AIX.patch
...
BuildRequires: automake = 1.15.1, autoconf = 2.69
...
if [ ! -f /opt/freeware/lib/libgo.a ]; then
    ln -sf ${BUILD}/gcc-build-%{gcc_version}/%{buildhost}/libgo/.libs/libgo.a /opt/freeware/lib/libgo.a
fi
%endif
...
%if %{with_go}
    ...
    cd /opt/freeware/src/packages/BUILD/gcc-build-%{gcc_version}
    cd %{buildhost}/libgo
    (gmake -k check || true)
    cd /opt/freeware/src/packages/BUILD/gcc-build-%{gcc_version}/gotools
...
echo "Tests 2) Compiler + libgo tests"
date
...
%changelog
* Tue May 17 2019 Clément Chigot <clement.chigot@atos.net> - 9.1.0-1
