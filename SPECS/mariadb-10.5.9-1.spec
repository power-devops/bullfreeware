...
## MYSQLCOMPAT does not work, and cannot be inactivated easily with CMake variable
Patch2:  mariadb-10.5.5-compat.patch
# Skip versioning and threadpool test and unskip some non failing test
Patch3: mariadb-10.5.5-skip_test.patch
## SI_KERNEL is not defined
Patch4: mariadb-10.4.6-SI_KERNEL.patch
## Wrong way to search altivec, plateform specific without else
Patch5: mariadb-10.5.9-crc32_altivec.patch
...
%prep
# If MariaDB is already installed on the machine, we can have conflicts during test.
# To be sure the built MariaDB is used, uninstall MariaDB from machine before.
if [ -n "`rpm -qa | grep mariadb-common`" ]; then
echo "Package mariadb-common is installed."
echo "It can conflicts with build during test. Exit."
exit 1
fi
...
%patch1 -p1 -b .krb5
%patch2 -p1 -b .mysqlcompat
%patch3 -p1 -b .skip_unskip_test
%patch4 -p1 -b .SI_KERNEL
%patch5 -p1 -b .crc32_altivec
...
%if %{with dotests}
# If root, create and use mysql user.
